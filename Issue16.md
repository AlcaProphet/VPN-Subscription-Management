...
> R30-02～R30-04 已按先前授权实施；用户于 2026-09-14 确认本节的三态 SMTP 方案并授权修复。以下区分代码/自动化与真实服务、人工作业，不把前者当成后者。
...
| 独立问题 R30-05 | OIDC Secret 不纳入本轮 SMTP 连接方式或邮件内容修改。 | 仍须隔离 OIDC 复现/回归，代码未改。 |
...
1. R30-02～R30-04 的单独修复授权已执行；平台 API 方案已撤回。本轮已按用户确认完成通用 SMTP 重设计与 R30-06 工程修复。
...
4. OIDC Secret 风险为本文件独立 R30-05，仍待单独处理；R30-06 代码及自动化已修复，真实浏览器/发信未验收。
