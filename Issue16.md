...
> **文档定位：** 记录 Build11 邮件人工核查的问题、2026-09-14 SMTP 实测发现的缺陷，以及用户已确认的邮件接入范围。本文是研究与分步实施方案，**尚未授权构建**；代码实施前仍须建立对应 Build 文档并逐 Step 执行。
...
- **状态：** ☐ 待修复；现场已重新输入密码恢复发送。

### R30-03 单一 SMTP 表单混淆端口与加密方式

- **现象与根因：** 现有 `tls` 布尔值表示“连接即 TLS”；关闭时先按明文 SMTP 连接，再在服务端宣告支持时尝试 STARTTLS。腾讯云 SES 的 587 端口在本次无凭据握手检查中表现为 TLS 直连，旧配置 `587 + TLS 关闭` 曾导致约 60 秒后端等待；前端统一 15 秒超时先显示网络异常。`465 + TLS 开启` 已通过用户实测。其他服务商的 587 不应套用腾讯云行为，例如 Mailgun、AWS SES、SendGrid 文档均列出 587 的明文握手/STARTTLS 路径。
...
> 每一步须在后续独立 Build 文档细化目标、前置条件、产出文件、参考实现和验收命令；按 AGENTS.md 一次执行一个 Step。本表不是已授权构建。
...
1. 本文记录已确认范围与实施提案，不构成开始编码授权；建立 Build 文档和逐步授权后再实施。
