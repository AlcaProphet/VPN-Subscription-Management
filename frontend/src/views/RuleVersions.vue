  ...
  <div>
    ...
    <template v-else>
      <div class="flex items-center gap-4 mb-5 flex-wrap">
        ...
        <h2 v-if="rule" class="m-0 text-xl font-semibold text-gray-900 dark:text-white">{{ rule.name }} — 版本管理</h2>
...
async function fetchRule() {
  ...
  try {
    const res = await adminApi.rules.get(id)
...
async function onFileUpload(file) {
  ...
  fd.append('file', file)
  try {
    await adminApi.rules.uploadVersion(id, fd)
    toastSuccess('版本已上传')
    await fetchRule()
  } catch (e) {
...
async function onTextSave(content) {
  const id = route.params.id
  try {
    await adminApi.rules.createVersionFromText(id, content)
    toastSuccess('新版本已创建')
    await fetchRule()
  } catch (e) {
...
async function handleSwitch(v) {
  const id = route.params.id
  try {
    await adminApi.rules.switchVersion(id, v.version)
...
async function handlePreview(v) {
  const id = route.params.id
  try {
    const res = await adminApi.rules.getVersion(id, v.version)
...
async function handleDeleteVersion() {
  ...
  const id = route.params.id
  try {
    await adminApi.rules.deleteVersion(id, deleteVersionTarget.value.version)
    toastSuccess('版本已删除')
