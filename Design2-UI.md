...
| 状态 | 内容 |
|------|------|
| 正常（已设置首页默认规则且有激活版本） | 左侧：卡片标题「分流规则」+ 规则名称 + 当前激活版本号；右侧：`a-button type="primary"`「复制规则链接」（点击即复制 + Toast）；**不展示完整 URL 输入框**，避免长链接挤压布局 |
| 空态（未设置默认规则 / 选中规则无激活版本） | **居中大标题**「分流规则为 Shadowrocket 客户端专用」+ **居中灰色说明**「使用指引：先添加订阅获取节点，再导入分流规则」；不再显示「管理员暂未设置分流规则」 |

- **SR 双内容导入引导文案**：仅在空态居中展示，已设置默认规则时不展示。
...
列：池名称、挂接 URL 数、条目数、上次同步时间（本地时区）、同步状态 Badge（色系见 1.2，failed 附原因 Tooltip；**数据源为 listPools 返回的 sync_status/sync_error 字段（最近任务完成后的快照，与 pool_sync_tasks 最近任务等价；服务重启中断任务显示 failed）**）、定时同步（`a-switch` 行内开关 + 时刻展示「每日 04:00 UTC」）、操作（详情 / 同步 / 编辑 / 删除）。**行内「同步」按钮**（loading 态防重复，见 5.2.3 轮询流）；定时开关行内切换即时保存，时刻编辑在池编辑弹窗内（`a-time-picker`，默认 04:00，副说明「按 UTC 每日执行，启动时补跑今日错过」）。
...
- **步骤条形态**：`a-steps` 按当前装配类型显示实际步骤，不再包含“类型与目标”步骤：Clash YAML 为 ① 头部表单 → ② 节点与代理组 → ③ 规则素材 → ④ 覆盖层 → ⑤ 预览；SR 节点订阅与通用节点订阅为 ① 头部表单 → ② 节点勾选 → ③ 预览；SR 分流规则为 ① 头部表单 → ② 规则素材 → ③ 预览。步间「上一步/下一步」，已填步骤可点击回跳；预览步提供「确认生成」按钮。
- **单页形态**：全部步骤转为纵向分区卡片（`a-card` + 锚点侧栏或顺序滚动），底部固定操作条「确认生成」；**两形态共享同一份表单状态与校验**
...
| ② 代理组配置 | 强制组（🚀直接连接 / 🌎国外流量 / 🛟无法归属的流量）均锁定勾选态（Checkbox 选中禁用 +「强制」Tag），但在每个组项旁明确展示成员能力：**🚀直接连接**显示「系统固定的直连出口，不提供成员选择」；**🌎国外流量**显示「可选成员：本次已勾选节点、🚀直接连接」并提供「选择与排序」；**🛟无法归属的流量**显示「可选成员：本次已勾选节点、🚀直接连接、🌎国外流量」并提供同形态「选择与排序」。两个成员弹窗均为有序多选（桌面可拖拽，所有尺寸提供上移/下移），只列本次已勾选且可用节点和允许引用的强制组；底层 `DIRECT` 不展示，弹窗提示「需要直连时请选择『🚀直接连接』」。成员配置分别写入 `overseas_members` / `fallback_group_members` 并随装配快照保存，重新编辑自快照恢复。其后为预设组库 `a-checkbox-group`（内置参考组，含组类型标注）与自建组列表（引用代理组管理页已建组，勾选 +「前往代理组管理」链接） |
...
| ④ 覆盖层 | Clash YAML 专属：Merge / Rules / Proxies / Groups 四个 `Input.TextArea`，提供空模板按钮与字段说明；预览前并入生成请求；重新编辑时从快照回填 |
| ⑤ 预览/确认生成 | 见 5.3.5；该步底部提供「确认生成」。**空组硬校验**（前端预校验 + 后端兜底）：「🌎国外流量」或「🛟无法归属的流量」成员为空时拒绝进入预览/生成，`a-alert error` 定位提示「『{组名}』组未包含任何成员，空组将导致 Clash 加载失败」；规则为空允许生成但预览区提示「未勾选任何规则，仅含兜底规则」 |

#### 5.3.2 SR 节点订阅装配器

- ① 头部表单：STATUS（预填建议格式：日期 + 版本标识，可改）+ REMARKS（预填站点名）+ 「一键采用默认值」
- ② 节点勾选：同 5.3.1 双来源分组列表（无代理组步骤）
- ③ 预览/确认生成：**空产物校验**：未勾选任何节点或**跳过不可转协议后有效链接数为 0** 时拒绝生成（提示「请至少勾选 1 个可转换节点」）；**不可转协议跳过提示**在预览/生成回执中列出（如 snell/openvpn 等，见 Design2.md §4.5）

#### 5.3.2a 通用节点订阅装配器

- ① 头部表单：**无**（generic-subs 不输出 STATUS/REMARKS 头部，Design2.md §4.5）
- ② 节点勾选：同 5.3.1 双来源分组列表（无代理组步骤）
- ③ 预览/确认生成：**空产物校验**：与 5.3.2 同口径（至少 1 个可转换节点）
...
- 顶部目标选择卡片中提供 `a-select` 列出全部规则实体（含无版本空实体，后缀「（空实体）」标注）+ 行内「新建空规则实体」快捷入口（弹窗仅名称 + 客户端类型固定 Shadowrocket，放宽首版校验，Design2.md §3.4）；同时支持「新建规则」模式直接输入规则名称（R22-08），生成时自动创建规则实体。
- 选中空实体时提示「该规则无激活版本，首个装配版本将自动激活」（Design2.md §4.4 首次自动激活）；新建规则模式生成后展示返回的 `rule_id`。
...
- 功能匹配：素材池（Design2 第二章）/ 顶部目标选择 + 按类型分步骤（§4.1，Clash 五步含覆盖层；SR subs/generic 三步；SR conf 三步）/ 头部表单（§4.2）/ 节点双来源（§3.2）/ 强制组与预设库（§3.3）/ 五类组类型与高级字段（§3.3/§7）/ SR 双装配器与通用装配器（§3.4）/ 规则全集与 SR 子集（§3.5）/ 覆盖层（§3.7）/ 兜底 FINAL（§3.6）/ diff jsdiff（§4.1）/ URI 批量导入（§6.2a）/ 入池引导与重新编辑（§4.4）均有着落
- 动线：新建池 → 同步轮询回执 → 手动条目 → 目标选择（条件展示）→ Clash 装配五步（含覆盖层）→ 预览 diff → 确认生成 → 去激活；重新编辑 → 悬空剔除 → 生成新版本
...
- PageHeader 操作区新增「批量导入」按钮；弹窗内多行 `Input.TextArea`，支持 ss / vmess / vless / trojan / anytls / hysteria2 / hysteria / tuic / wireguard / http(s) / socks5，也支持整块 Base64 订阅文本。
- 提交后展示逐行回执表格：行号 / 名称 / 成功或跳过 / 原因；成功后刷新节点列表。
...
列：组名称、类型 Tag（`a-tag`：preset 蓝「预设」/ custom 绿「自建」）、组类型（select / url-test / fallback / load-balance / relay 代码风格 Tag）、成员摘要（N 个节点 · M 个子组）、启用勾选（仅预设组，见下）、操作（编辑 / 删除——预设组不可删；**全部组名创建后不可改**，自建组编辑弹窗名称只读）。
...
1. **基本信息**：名称（**创建后不可修改**：新建可输入、编辑只读，后端拒绝改名；重名 409 提示——重名范围含其他组名、任一节点有效渲染名、强制组名与 Clash/mihomo 内建保留代理名；禁止控制字符、逗号与首尾空白，允许中文/emoji）+ 组类型 `a-select`（select 默认 / url-test / fallback / load-balance / relay，附各类型一句话说明；**名称只读、组类型创建后可修改**；预设组编辑时名称/preset_key 只读，组类型同样可改）
1a. **高级字段（折叠区）**：Provider 引用 `use`、健康检查 `url` / `expected-status` / `interval` / `timeout` / `max-failed-times`、`lazy` / `disable-udp` / `interface-name` / `routing-mark`、`filter` / `exclude-filter` / `exclude-type`、`include-all` / `include-all-proxies` / `include-all-providers`、`hidden` / `icon`；按组类型允许范围保存与校验。
...
- 功能匹配：强制组/预设库/自建组（Design2.md §3.3）、组类型五枚举、高级字段（use/健康检查/过滤/include-all 等）、有序节点引用与 select 首项语义、DAG 校验、组内容约束、**名称创建后只读与字符集校验**均有着落；预设组种子默认成员「🚀直接连接」；强制组不在本页管理（系统内置渲染结构、不入 proxy_groups 表，仅装配器锁定展示，见 Design2.md §3.3 强制组落库口径）
...
| 跨页装配草稿 | 仅从装配构建前置条件跳转时保存；`sessionStorage('assembly_ctx_v1')` 有效期 **30 分钟**，包含 `overseas_members` / `fallback_group_members`，回到装配页恢复一次后即删除，过期/损坏草稿不显示提示也不恢复 |
