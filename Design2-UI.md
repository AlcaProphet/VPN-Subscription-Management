...
| 页头类型 Tag | **订阅版本页隐藏页头类型 Tag**（标题已含“资源名 · 版本管理”，避免重复）；规则 / 分享 / 自定义订阅版本页仍显示 `owner.type_label` 类型标签 |
...
- **「首页默认」设置**：列表**不设独立「首页默认」单选列**；设置/取消首页默认统一收口到操作区——非默认行操作区显示「**设为首页默认**」（ConfirmModal「设为首页默认后，原默认规则『{旧规则名}』将自动取消默认」），默认行操作区显示「**取消默认**」（ConfirmModal「取消后首页分流规则卡片将回到未设置空态」）；移动端卡片同样在操作区展示，不渲染 Radio。后端 `is_home_default` 唯一性与 `setHomeDefault` 事务语义不改变
- **客户端类型列**：管理员规则列表**不展示「客户端类型」列**，移动端卡片不重复展示 `client_type` 标签；创建表单仍保留 Shadowrocket 唯一类型约束，后端字段、装配识别与用户端规则标签保持不变
...
- **目标类型页签文案**：四个目标 Tab 显示面向用户文案：`Clash - V2Ray/Mihomo（新版）`、`Shadowrocket 订阅组`、`通用V2Ray格式`、`Shadowrocket规则组`；内部 `TargetSyntax`、路由 query、API 值与后端枚举保持不变
...
| 步骤 | 规格 |
|------|------|
| ① 头部表单 | Clash 顶层键表单（字段范围见 Design2.md §4.2），**默认值以作者个人配置头部预填** + 「使用默认值」警示按钮（覆盖已改字段前 ConfirmModal「将覆盖当前已填头部字段」）；右上角用 Switch 在「结构化表单 / 高级 JSON」双态间切换；长表单折叠分区（基础键 / DNS / 其他） |
| ② 节点勾选 | 双来源分组 Section/Card：**“手动添加的节点”**区（含协议 Tag、已选/总数摘要、中文空态）与 **Xray 节点**区（高级模式且有检测节点时展示；基础模式/无实例时 xray 区空态文案「未检测到 Xray 节点（高级模式录入实例后手动刷新节点发现）」）；xray 区节点行展示有效渲染名（display_name 非空则用之，否则系统名；有自定义名时副行 `code` 风格展示系统标识名）；**allocatable=0 节点行置灰不可勾 + Tooltip「该协议无 per-user 能力，不可分配」**；missing=1 节点不列入候选；选项行统一为边框行，已选高亮、不可用/失效状态单独标注 |
| ② 代理组配置 | 代理组区使用 Section/Card 容器与已选摘要（普通组），选项行统一边框并区分已选/强制/自建/失效状态；强制组（🚀直接连接 / 🌎国外流量 / 🛟无法归属的流量）均锁定勾选态（Checkbox 选中禁用 +「强制」Tag），但在每个组项旁明确展示成员能力：**🚀直接连接**显示「系统固定的直连出口，不提供成员选择」；**🌎国外流量**显示「可选成员：本次已勾选节点、🚀直接连接」并提供「选择与排序」；**🛟无法归属的流量**显示「可选成员：本次已勾选节点、🚀直接连接、🌎国外流量」并提供同形态「选择与排序」。两个成员弹窗均为有序多选（桌面可拖拽，所有尺寸提供上移/下移），只列本次已勾选且可用节点和允许引用的强制组；底层 `DIRECT` 不展示，弹窗提示「需要直连时请选择『🚀直接连接』」。成员配置分别写入 `overseas_members` / `fallback_group_members` 并随装配快照保存，重新编辑自快照恢复。其后为预设组库 `a-checkbox-group`（内置参考组，含组类型标注）与自建组列表（引用代理组管理页已建组，勾选 +「前往代理组管理」链接） |
...
- ① 头部表单：STATUS（预填建议格式：日期 + 版本标识，可改）+ REMARKS（预填站点名）+ 「使用默认值」警示按钮
...
- ① 头部表单：`[General]` 键值表单（bypass-system / skip-proxy / dns-server 等，预填默认值，可通过「使用默认值」警示按钮一键恢复）
...
**头部参数控件分区（所有目标统一）**：文本、数字和选择型头部参数保留在“头部参数”网格；所有 bool 型头部参数独立进入“开关参数”区域，采用统一的 Switch、标签、焦点与点击区域布局。该规则适用于 Clash YAML 的 `Allow LAN`、SR 分流规则的 `IPv6` 及后续新增的 bool 型头部参数；仅改变表单组织，不改变 `fixed_params_text` 的字段名、默认值或生成语义。头部右上角使用 **Switch 在「结构化表单 / 高级 JSON」双态间切换**；默认值操作为「**使用默认值**」警示按钮，仍保留“覆盖当前已填头部字段”的确认弹窗，**不改变整体覆盖语义**。
...
1a. **高级字段（折叠区）**：Provider 引用 `use`、健康检查 `url` / `expected-status` / `interval` / `timeout` / `max-failed-times`、`lazy` / `disable-udp` / `interface-name` / `routing-mark`、`filter` / `exclude-filter` / `exclude-type`、`include-all` / `include-all-proxies` / `include-all-providers`、`hidden` / `icon`；按组类型允许范围保存与校验。**高级开关项统一使用单行 `flex` 布局（Switch + 文案不换行），窄屏降为单/双列，避免“引入全部 Provider”等长文案折行。**
