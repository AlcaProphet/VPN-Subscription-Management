...
| 14 | R27-09 全链路回归、固定版本证据、浏览器与文档收口 | ◐ 执行中：工程问题见 Issue14；人工项目及结果见 ProdTestList |
...
| 14 | 前后端相关测试、`Design4.md`、`Issue14.md`、`ProdTestList.md`、`AGENTS.md`、本文件 | 全量验证与固定版本正反例；工程问题归 Issue14，人工边界归 ProdTestList，不宣称 Shadowrocket 真机兼容已完成 |
...
- **TODO：**
  - [ ] 按 Issue14 完成工程性问题、正式 smoke 夹具和固定版本验收门禁收口。
  - [ ] 按 ProdTestList 完成用户人工项目并记录环境、日期、结果和证据。
...
- **本轮执行记录（2026-09-08，尚未验收）：** 后端全量测试、指定竞态测试、`go build ./...`、`go vet ./...`、前端 41 文件/209 用例与生产构建均通过；四类装配器、URI 导入（2 ok/1 skip）、覆盖层及 v2 导出/导入在临时修正执行流中通过。固定 Mihomo 的此前定向执行记录曾在显式 `MIHOMO_11929_BIN` 环境通过，但本次全量核验发现未设置该变量时测试会 `SKIP`，该证据门禁缺口转 [Issue14.md](Issue14.md) R28-04。真实 API 浏览器流程覆盖 Setup、管理员注册、四个已知 SS 插件/未知插件切换、未知 `password`/`token` 普通参数、目标检查、保存重开、敏感凭据“已保存（留空保留）”和桌面/375px 无横向溢出；动态插件输入操作捕获两条 Ant Design 输入组件异常，工程问题转 Issue14 R28-01。正式 smoke 夹具问题转 Issue14 R28-02/R28-03；用户人工项目转 [ProdTestList.md](ProdTestList.md)。
- **R28-01 修复记录（2026-09-09，Step 14 仍在执行）：** 失败优先的父子闭环回归确认未知插件参数名每次改名都会改变 Map 行 Vue `key`，使旧 Ant Design Input 在回调完成前卸载。`ProtocolFieldEditor` 改为维护与参数名解耦的本地稳定行身份，新增、改名、父级/外部回填、删除和分支清空时同步对齐，不改变即时模型回写、校验、API 或保存结构。组件宿主和 `NodesView` 回归均覆盖逐字符输入、DOM/焦点保持及最终原子改名；定向 2 文件/53 用例、前端全量 41 文件/210 用例、生产构建、后端全量测试、编译和 vet 通过。R28-02～R28-04 与 ProdTestList 人工门槛尚未完成，因此 Step 14 不在本次标记验收通过。
- **R28-02 修复记录（2026-09-09，Step 14 仍在执行）：** 失败优先矩阵确认通用 `J()` 会将 JSON 布尔 `false` 输出为 Python `False`，旧小写字符串比较误拒绝正常响应，却会接受错误类型的字符串 `"false"`。10d 改为直接解析 `data.emergency` 并以 `value is False` 严格校验 JSON 布尔类型和值，保留规范诊断输出且不修改通用提取器或服务端合同；脚本语法、仅 `false` 通过的输入矩阵、服务端普通/应急状态定向测试、后端全量测试/编译/vet及前端生产构建均通过。R28-03～R28-04 与 ProdTestList 人工门槛尚未完成，正式 Production smoke 全链路复验待 R28-03 后执行，因此 Step 14 仍未验收。
- **R28-03 修复记录（2026-09-09，Step 14 仍在执行）：** 完整影响核对确认步骤 13 基础 Clash 与步骤 13f 覆盖层 Clash 两处请求均遗漏当前必需的 `fallback_group_members`。两处均按既有前端初始值和强制组合同显式补入 `["🚀直接连接","🌎国外流量"]`，未修改后端、前端或接口合同。脚本语法、强制组合同定向测试、后端全量测试/编译/vet及前端生产构建通过；未做临时转换的正式 `bash .smoke-test-prod.sh` 已完整通过四类装配器、URI 导入 2 ok / 1 skip、覆盖层与 v2 导出/导入，并输出双完成标记。R28-04 与 ProdTestList 人工门槛尚未完成，因此 Step 14 仍未验收。
