...
// SubmitSync 提交同步任务：池存在性 + 无 running 检查 + 插入任务在同一 BEGIN IMMEDIATE 事务内完成；
// 事务提交后启动 goroutine 执行，返回任务 ID
func (s *Service) SubmitSync(ctx context.Context, poolID int64) (int64, error) {
	...
	go s.runSyncTask(poolID, taskID) // 后台执行，不阻塞请求
	return taskID, nil
}
...
// ListTasks 历史任务分页（id DESC，保留 7 天）
...
// runSyncTask 后台执行：串行拉取全部 URL → 成功项 upsert → 全部成功才差量删除；
// 池已删除时仅记日志不崩溃；编辑 URL 只影响下一次同步
func (s *Service) runSyncTask(poolID, taskID int64) {
	ctx := context.Background()
	var urlsRaw string
	if err := s.store.DB().QueryRowContext(ctx,
		`SELECT urls_json FROM rule_pools WHERE id = ?`, poolID).Scan(&urlsRaw); err != nil {
		s.log.Warn("同步任务启动失败：素材池不存在", "pool_id", poolID, "task_id", taskID, "err", err)
		return
	}
	...
	err := s.store.TxImmediate(ctx, func(tx *sql.Tx) error {
		...
		// 成功 URL 条目照常 upsert（manual 冲突行跳过，manual 段永不受同步改写）
		for _, r := range results {
			...
			for _, e := range r.entries {
				...
				if _, err := tx.ExecContext(ctx,
					`INSERT INTO pool_entries (pool_id, rule_type, match_value, source, sort_order)
					 SELECT ?, ?, ?, 'url', ?
					 WHERE NOT EXISTS (
					   SELECT 1 FROM pool_entries WHERE pool_id = ? AND rule_type = ? AND match_value = ? AND source = 'manual'
					 )
					 ON CONFLICT(pool_id, rule_type, match_value) DO UPDATE SET
					   match_value = excluded.match_value, updated_at = CURRENT_TIMESTAMP`,
					poolID, e.RuleType, e.MatchValue, order,
					poolID, e.RuleType, e.MatchValue); err != nil {
					return err
				}
			}
		}
	...
	if err != nil {
		...
		_ = s.store.TxImmediate(ctx, func(tx *sql.Tx) error {
			...
			_, e := tx.ExecContext(ctx,
				...
				toJSON(stripEntries(results)), truncateError(finalErr, 200), taskID)
			if e != nil {
				return e
			}
			_, e = tx.ExecContext(ctx,
				`UPDATE rule_pools SET last_synced_at = CURRENT_TIMESTAMP, sync_status = 'failed', sync_error = ? WHERE id = ?`,
				truncateError(finalErr, 200), poolID)
			return e
		})
	}
}
...
func (s *Service) syncURL(ctx context.Context, client *http.Client, target string) (r PerURLResult) {
	...
	entries, skipped := parseURLBody(body)
	if len(entries) == 0 {
...
// parseURLBody 逐行解析并按（类型,值）去重合并
func parseURLBody(body []byte) ([]parsedEntry, int) {
	...
	skipped := 0
	for sc.Scan() {
		typ, val, _, ok := ParseLine(sc.Text())
		if !ok {
			skipped++
			continue
		}
		norm, err := ValidateEntry(typ, val)
		if err != nil {
		...
		seen[key] = true
		out = append(out, parsedEntry{RuleType: typ, MatchValue: norm})
	}
	return out, skipped
}
