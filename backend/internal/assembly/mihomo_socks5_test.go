...
// TestMihomo11931Socks5ProtocolStructures 覆盖 SOCKS5 显式 adapter 在固定 Mihomo v1.19.31
// 上的代表性正反例；普通 go test 未设置外部二进制时跳过，严格入口由 .mihomo-test.sh 负责。
func TestMihomo11931Socks5ProtocolStructures(t *testing.T) {
	bin := os.Getenv("MIHOMO_11931_BIN")
	if bin == "" {
		t.Skip("未设置 MIHOMO_11931_BIN，跳过固定 Mihomo 1.19.31 SOCKS5 验收")
	}
	...
	t.Run("accepts generated structures", func(t *testing.T) {
		raw, err := os.ReadFile(filepath.Join("testdata", "node_check", "socks5-basic-tls.json"))
		if err != nil {
		...
		proxy := new(Service).clashProxy(&nodeData{
			Protocol: fixture.Protocol, RenderName: "fixed-socks5-basic-tls",
			Host: fixture.Host, Port: fixture.Port, ProtocolJSON: fixture.ProtocolJSON,
		})
		content := mihomoSSPluginConfig(t, orderedMapToMapSlice(proxy), "fixed-socks5-basic-tls")
		output, err := runMihomoConfigTest(t, bin, content)
		if err != nil {
			t.Fatalf("Mihomo 1.19.31 拒绝生成的 SOCKS5 结构: %v\n%s\nYAML:\n%s", err, output, content)
		}
	})
	...
	t.Run("rejects invalid structures", func(t *testing.T) {
		cases := []struct {
			...
			{
				name: "socks5 invalid client certificate",
				proxy: gyaml.MapSlice{
					{Key: "name", Value: "bad-socks5-cert"}, {Key: "type", Value: "socks5"},
					{Key: "server", Value: "192.0.2.1"}, {Key: "port", Value: 1080},
		...
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				...
				output, err := runMihomoConfigTest(t, bin, content)
				if err == nil {
					t.Fatalf("Mihomo 1.19.31 应拒绝无效 SOCKS5 结构 %s\n%s", tc.name, content)
				}
