...
proxies: # socks5
  ...
  - name: "snell"
    ...
    # obfs-opts:
    #   mode: http # or tls/shadow-tls/restls/jls
  ...
  - name: "vmess"
    ...
    #     primary-egress-outbound: "" # mihomo VMess outbound 保持为空；v2ray 可填写专用的控制出站 tag
    #   sequence-watermarking-enabled: false # 启用序列水印
    #   embedded-traffic-generator: # 生成额外的 HTTP 载体流量，协议由 alpn 决定
    #     steps:
    #       - host: example.com
    #         path: /
    #         method: GET
    #         connection-ready: true # 此步骤完成后再交付代理连接
    #         connection-recall-exit: true # 代理连接关闭后退出载体流量
    #         h2-do-not-wait-for-download-finish: false # h2 下不等待响应体读取完成
    #         wait-time:
    ...
    #   config: AEn+DQBFKwAgACABWIHUGj4u+PIggYXcR5JF0gYk3dCRioBW8uJq9H4mKAAIAAEAAQABAANAEnB1YmxpYy50bHMtZWNoLmRldgAA
    #   # query-server-name: xxx.com # 可选项，不为空时用于指定通过dns解析时的域名
    # shadow-tls-opts: # 需要 tls: true；使用 servername 作为 ShadowTLS SNI
    #   version: 3 # 支持 v1/v2/v3；留空时默认为 v2
    #   password: shadow-tls-password
    # restls-opts: # 需要 tls: true；使用 servername 作为 Restls SNI
    #   password: restls-password
    #   version-hint: tls13 # 可选值：tls12、tls13
    #   # restls-script: ""
    # jls-opts: # 需要 tls: true；使用 servername 作为 JLS SNI
    #   username: jls-user
    #   password: jls-password
    # reality-opts: ...
    # ws-opts:
  ...
  - name: shadowquic
    ...
    # zero-rtt: false # 官方 server 的 0-RTT 路径可能在 JLS 认证完成前读取用户状态（已确认影响 v0.3.11）；使用官方 server 如果遇到不通或断联情况，请在服务端关闭 zero-rtt
    # keep-alive-interval: 10000
    # congestion-controller: bbr # Available: "cubic", "new_reno", "bbr". Default: "cubic"
    # up: 100 Mbps # enables brutal negotiation via a mihomo private extension; client upload speed; falls back to congestion-controller when unsupported
    # down: 100 Mbps # enables brutal negotiation via a mihomo private extension; requested client download speed, capped by listener up; falls back to congestion-controller when unsupported
    # cwnd: 10 # default: 32
    # bbr-profile: "" # Available: "standard", "conservative", "aggressive". Default: "standard"
  ...
  - name: sudoku
    ...
    # multiplex: "off" # 可选：off（默认）、auto（仅复用 HTTPMask 底层连接）、on（在原始 TCP 或 HTTPMask 上启用 Sudoku 单会话多目标 mux）
...
proxy-providers:
  provider1:
    ...
    override: # 覆写节点加载时的一些配置项
      ...
      #   - pattern: "IPLC-(.*?)倍"
      #     target: "iplc x $1"
      # # 这是 yq v4 风格的配置覆盖子集。数组项按顺序执行，且晚于上面的固定字段覆盖。
      # # 路径：.、.name、."a.b"、.["a.b"]、.items[0]、.items[-1]。[] 可以遍历数组或 mapping。
      # # 读取缺失路径或穿过标量时返回 null。赋值会创建缺失的 mapping 和非负数组索引，但不会穿过标量更新。
      # # 赋值：=、|=、+=、-=、*=。删除：del(.field)、del(.a, .b)。
      # # 值：null（或 ~）、布尔值、十进制/十六进制整数、浮点数、双引号字符串、数组及双引号键的 mapping。
      # # 负号只用于十进制整数和浮点数。浮点数支持科学计数法。
      # # 字符串按 yq 规则解码 \n、\"、\\；其他反斜杠序列保留原文。
      # # 数组会收集每一项的全部结果。mapping 的 value 为零结果时省略 key，多结果时保留最后一个。
      # # 运算：+、-、*、/、%、==、!=、<、<=、>、>=、and、or、//（默认值）。
      # # + 可拼接字符串/数组或浅合并 mapping；* 可做数乘、字符串重复、数组替换或深合并 mapping；/ 可分割字符串。
      # # 流：管道 |、逗号 union、select。使用 [...] 将零到多个结果收集为数组。
      # # 字符串支持 "[\(.type)] \(.name)" 形式的插值。插值表达式有多个结果时使用第一个。
      # # 字符串外的 # 会忽略该数组项中的后续表达式内容。
      # # 查询函数：length、keys、has(key/index)、contains(value)、select(condition)。字符串 length 按 UTF-8 字节数计算。
      # # 数组函数：reverse、sort、unique、flatten([depth])、any、all。
      # # 转换函数：map、map_values、filter、to_entries、from_entries、with_entries、any_c、all_c。
      # # map/filter/map_values 支持数组或 mapping。with_entries 只支持 mapping，any_c/all_c 只支持数组。
      # # map/filter 返回数组，map_values 保持输入集合类型。to_entries(null) 不产生结果。
      # # from_entries 只接受具有字符串 key 的 entries 数组。
      # # 字符串函数：test(pattern[, "g"])、sub(pattern, replacement)、split(separator)、join(separator)、upcase、downcase、trim。
      # # split(null) 不产生结果，join 只接受数组。
      # # 类型函数：tostring、tonumber、type、not。tonumber 支持十进制、0x 十六进制、0o 八进制和浮点字符串。
      # override-expr:
...
listeners:
  ...
  - name: shadowquic-in-1
    type: shadowquic
    port: 10822 # 支持使用ports格式，例如200,302 or 200,204,401-429,501-503
    listen: 0.0.0.0
    # routing-mark: 0 # 为监听socket设置routing-mark（仅支持linux）
    # rule: sub-rule-name1 # 默认使用 rules，如果未找到 sub-rule 则直接使用 rules
    # proxy: proxy # 如果不为空则直接将该入站流量交由指定 proxy 处理 (当 proxy 不为空时，这里的 proxy 名称必须合法，否则会出错)
