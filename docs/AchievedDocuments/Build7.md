...
- ◧ Step 1：独立 Xray 账号后端 + 实例级账号对账后端（Issue3 R17-01/02 未闭环，待复验）
- ◧ Step 2：高级设置后端、OFF 清空与配置导入导出 format_version=2（Issue3 R17-06 测试缺口未闭环，待复验）
- ◧ Step 3：Xray 实例页前端（实例/检测/初始化/对账/独立账号）（Issue3 R17-04/06 未闭环，待复验）
- ◧ Step 4：用户组/用户管理/面板设置高级前端（Issue3 R17-03/05/06/08 未闭环，待复验）
...
| Step | 内容 | 设计依据 | 状态 |
|------|------|---------|------|
| 1 | 独立账号 + 实例级对账后端 | Design2 §5.10/§5.11 | ◧ 进行中（Issue3 R17-01/02 未闭环） |
| 2 | 高级设置后端 + OFF 清空 + 导入导出 v2 | Design2 §一/§5.4/§5.10 | ◧ 进行中（Issue3 R17-06 测试缺口未闭环） |
| 3 | Xray 实例页前端 | Design2-UI §8 | ◧ 进行中（Issue3 R17-04/06 未闭环） |
| 4 | 组/用户/设置高级前端 | Design2-UI §4.3/§4.5/§4.7 | ◧ 进行中（Issue3 R17-03/05/06/08 未闭环） |
