...
原研究曾记录“同协议切换保留隐藏值、只输出当前激活值”的候选；Design4 v1.2 已确认改为切换即清空所属分支，不保存非激活数据。允许增加独立 Xray target adapter、条件模型覆盖更多协议、复杂扩展进入高级或兼容区域作为后续方向继续有效。

当前 Design4 已确定：数据库/API 形状为 `nodes` 行内最小当前状态 + 活动 `protocol_json` + 编辑修订号；首批 VLESS/VMess/Trojan/SS 动态表单与统一保存契约；其余协议过渡保留原表单。本文以下段落中与上述结论不一致的旧候选仅供理解研究演进，不作为实施要求。
...
| 状态 | 示例 | 保存、校验和输出 |
|---|---|---|
| 折叠但有效 | 已启用的 SMux 面板收起 | 继续校验、继续参与输出；折叠不清空 |
| 被切离的分支 | `network=grpc` 时的 WS 参数 | v1.2 已确认清空所属参数/凭据/扩展；不做当前连接的必填校验，也不保存恢复副本 |
| 对当前目标无法表达 | WS 自定义 Header 不能被某个 URI 映射完整承载 | 保留当前编辑值；给目标差异回执，不能静默算作完整支持 |
...
早期研究假设“保留隐藏值并区分激活/非激活”需要独立编辑状态。Design4 v1.2 已确认改为“切换即清空”的简化模型：
...
如果保留 B 中的 reality-opts，重开时无法区分 A/B，且当前 VLESS 链接逻辑还可能把 B 按 REALITY 输出。v1.2 的解决方式是不保存非激活分支：从 REALITY 切到 TLS 时清空原安全分区，切回 REALITY 需重新填写。当前状态仍必须显式保存（不能仅靠字段存在性猜测），但它只保存当前选择，不保存可恢复副本。

### 4.2 已确认的存储模型

v1.2 不再采用“独立编辑状态 + 非激活分支”的路线 B，也不是“单 JSON 全集 + 元数据”，而是：

| 数据 | 存储位置 | 说明 |
|---|---|---|
| 活动参数 | `nodes.protocol_json` | 唯一活动配置真值 |
| 当前选择元数据 | `nodes.current_state_json` | 显式 network/security/plugin/features，不保存分支副本 |
| 未知扩展 | `nodes.extensions_json` | 独立加密块，随所属分支清空 |
| 修订/格式版本 | `nodes.edit_revision`、`nodes.state_format_version` | 并发冲突检测与结构解释 |

数据流如下：

```mermaid
flowchart TD
    A[编辑草稿：当前选择 + 当前参数 + 重置范围] --> B[服务端归一化与凭据合并]
    B --> C[校验当前连接语义]
    C --> D[活动 protocol_json + 当前状态 + 扩展]
    D --> F[按目标投影与能力检查]
    F --> G[Clash / SR URI / generic URI / 候选 Xray outbound]
```

D 中不保存非激活分支，也不保留 A → B → A 的恢复副本。

### 4.3 分支清空与保存边界（v1.2 确认口径）

| 操作 | 已确认行为 |
|---|---|
| WS → gRPC → WS | WS 参数清空，切回 WS 需重新填写；不恢复 |
| REALITY → TLS → 关闭后重开 | 当前仍为 TLS；REALITY 参数已清空，不恢复 |
| 折叠 TLS 面板 | 不改变 TLS 是否激活，不清空有效值 |
| 非激活分支缺少连接必填项 | 该分支已被清空，不存在“非激活校验”问题 |
| 未应用局部 JSON 草稿 | 切换分支时丢弃对应草稿；切换编辑模式不清空有效值 |
| 更换插件／认证版本 | 旧分支参数/凭据/扩展清空；活动凭据只属于新分支 |
| 切换协议 | 清空全部协议参数与凭据，保留名称/服务器/端口等基础信息 |

保存层仍要检查 JSON 形状、敏感字段处理和数据版本；但不再需要为“非激活分支”保存数据或校验。
...
4. 旧 API 调用方缺少当前状态时的 PUT 语义需按统一保存契约处理：必须携带 `base_revision`、`reset_scopes` 与凭据/扩展操作；不允许再把旧格式整替换视为保留非激活分支。
5. URI 导入和表单创建使用同一归一化与校验入口，避免导入绕过状态模型；导入回执仍保留逐行结果。
6. `protocol_json`、当前状态、扩展和修订号在同一事务内更新，防止半更新；不把客户端传来的“已验证”标志当作服务端事实。
...
数组凭据需要稳定身份：Peer 删除／重排后，不能按旧数组索引合并到另一个 Peer。Design4 v1.2 使用 `credential_ops` 显式引用路径；后续 WireGuard/TUIC 等数组场景仍需继续设计稳定条目身份，不能按重排后的数组索引保留他项凭据。

加密、脱敏、编辑合并和输出解密应覆盖同一组敏感路径，包含对象数组和可能内嵌密钥的配置全文。REALITY 公钥不是服务端私钥；客户端合法 mTLS 私钥又不同于从服务端配置误抄的私钥，两者不能混为一个“禁止私钥字段”的规则。
...
| 被切离分支的旧字段 | v1.2 已确认清空，不保存到可恢复分支 | 不进入当前产物 |
...
【D8 已确认】未知扩展整体加密保存，标明来源/所属分区/目标；读取只返回摘要；支持保留/替换/清除；所属分支清空时连同删除。对已经由旧版本透传的扩展键，需要先做产物差异提示，不能在后台升级时一律静默删除。
...
|---|---|---|
| A：状态与兼容 | 当前状态/重置范围/凭据操作样例，DTO 草案，旧节点迁移示例 | 证明保存重开后维持当前选择，切回不恢复 |
| B：能力与校验 | 首批协议组合表、目标字段清单、正反例 | 证明规则不误杀合法缺省 |
| C：交互规格 | 桌面／移动布局、折叠与错误定位、JSON 应用流程 | 说明每次切换清空哪些数据 |
| D：实施准备 | 正式增量 Design 与分步 Build | A～C 已决策、影响范围明确，才进入构建 |
| E：扩展目标 | 更多协议与独立 Xray adapter 的独立方案 | 固定版本并通过适配器验收 |

界面示意可以提前绘制；但“切换即清空、当前状态投影”不能在没有服务端状态与输出投影时提前标记为交付。
...
以下原为分析建议；Design4 v1.2 后多数已确认，形成当前结论。

| 编号 | 当前结论 | 原备选/已排除 | 决策影响 |
|---|---|---|---|
| D1 开关布局 | 独立开关区（用户明确指定） | 按语义分区、区内集中为研究候选 | 界面按 Design4 §3.3 |
| D2 保存模型 | `nodes` 行内当前状态 + 活动 `protocol_json`；取消独立状态表 | 独立编辑状态与非激活分支已排除 | API、数据库、凭据与历史兼容按 Design4 §12 |
| D3 跨协议切换 | 首轮保持整体重填；同协议传输/安全/插件切换也清空且不恢复 | 会话内分协议草稿已排除 | 清空规则见 Design4 §3.4 |
| D4 校验强度 | 确定性错误阻止保存，远端匹配与版本未知分级提示/警告 | 宽松仅警告或统一严格必填均不采用 | 保存与生成两级错误契约 |
| D5 首批协议 | VLESS／VMess／Trojan／SS，附 TUIC／WG 设计验例 | 一次覆盖 19 个未作为首批 | 首次实施范围 |
| D6 目标预览 | 新增节点检查接口，按需打开脱敏检查 | 只在装配提示未采用 | 检查端点见 Design4 §12.3 |
| D7 凭据语义 | 保留／替换／清除，通过 `credential_ops` 和 `reset_scopes` 显式表达 | 空串隐式保留不足以表达清除 | 敏感字段 API 见 Design4 §12.3 |
| D8 未知参数 | 整体加密扩展，来源/分区/目标明确，支持保留/替换/清除 | 全量透传或全部拒绝均不采用 | 高级 JSON 和迁移策略见 Design4 §12.1 |
| D9 Xray 范围 | 独立 outbound 导出＋固定验证 profile 为后续专项 | 全配置编辑／反向导入不作为首轮 | 单独立项，不并入首轮节点 UI |

### 11.2 应验证的行为，而非只检查控件数量

| 场景 | 期望结果 |
|---|---|
| VLESS WS → gRPC → 保存 → 重开 → WS | gRPC 保持当前状态；WS 参数已被清空、不恢复；各产物仅含当前激活参数 |
| REALITY → TLS → 保存 → 重开 | 普通 TLS 不被残留公钥或空 `reality-opts` 重新激活；再切回 REALITY 需重新填写 |
...
| TUIC v4/v5 切换 | 后续协议设计认证互斥；所属旧分支参数清空，不保存可恢复分支 |
