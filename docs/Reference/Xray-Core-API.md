...
> **文档定位：** 本文档是 Xray-core gRPC API 的研究结论与参考资料库，供 vpn-sub 项目 Xray 对接（高级模式）开发与排查时查阅。设计决策与方案见 [Design2.md](../reports/Design/Design2.md) 第五章；SSPanel-UIM 参照研究见 [SSPanel-Research.md](SSPanel-Research.md)；节点链接标准见 [Node-Link-Standards.md](Node-Link-Standards.md)。
