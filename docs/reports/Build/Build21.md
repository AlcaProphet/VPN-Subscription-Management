...
> **文档定位：** 本文档是依据 [BuildReport3.md](../BuildReport/BuildReport3.md) 开展的第二十一轮构建记录，将“Build17~Build20 表单结构与 Design4 对齐”发现的问题按用户确认的推荐方案落地。**本文件已归档至 `docs/reports/Build/Build21.md`，仅用于核查，不再作为当前执行入口。**
> - 设计依据：[Design4.md](../../../Design4.md)（当前最新节点编辑器设计）、[BuildReport3.md](../BuildReport/BuildReport3.md)
> - 编码指令：[AGENTS.md](../../../AGENTS.md)（**唯一强要求**）
> - 前序构建：[Build17.md](Build17.md)～[Build20.md](Build20.md)、历史构建存档于 [docs/reports/Build/](.)
>
> **本文件状态：** **已归档（2026-09-10 文档交叉审核）；Build21 自身 Step 1～15 无剩余工程任务，后续问题由 Issue14 承接。** 原 Step 1～6 已完成并通过验收；R27-09 全量扩展 Step 7～14 已实施并通过验收，Step 14 的 R28-01～R28-04、正式 Production smoke、固定 Mihomo 1.19.29 证据门禁和用户 PT-28-01～PT-28-05 均已收口。Issue14 中 R28-05～R28-09 属于后续项目级工程问题，不回写为 Build21 未完成；当前仍需跟踪的历史人工项目见 [ProdTestList.md](../../../ProdTestList.md)，结果以该清单为准。原 Build23 中的 R27-09 主体步骤已并入本节，新增的 N-node-3/N-node-4 SR TLS 增量 Step 15 已实施并通过验收。Build23 不再重复这些主体步骤，仅保留交接与差异说明。原 Build21 验收事实继续保留，不以补充计划倒写为“未完成”。
...
| 14 | 前后端相关测试、`Design4.md`、`Issue14.md`、`ProdTestList.md`、`AGENTS.md`、本文件 | 全量验证与固定版本正反例；工程问题归 Issue14，当前仍待人工复验的边界归 ProdTestList；PT-28 的 Shadowrocket/浏览器/Production 人工项目已确认完成 |
...
- BuildReport4 中不属于 Build21 R27-09 主体的遗留工程项（Build16/Design3 D3-1～D3-10、N-core、N-node-6、安全 N01～N07 等）已统一登记至 [Issue14.md](../Issue/Issue14.md) R28-05～R28-09；Build22 为 D3 实施计划。
...
- **验证边界：** 浏览器使用本轮生产前端构建、真实后端接口及全新临时 Dev 数据库；临时 Go overlay 仅将测试后端监听限制到 `127.0.0.1`，未更改认证/业务行为。该阶段目标检查面板交互及固定版本客户端导入/连接属于人工边界，现已由用户在 PT-28-01～PT-28-05 中确认完成；R27-05 额外尺寸/焦点/折叠和其他历史后续项目仍见 ProdTestList。新增输出测试发现 VMess SR 适配器既有产物没有显式 `tls` 查询参数，后续已由 Step 15 补齐并完成自动化回归。
...
- **环境与边界：** 全新临时 Dev 数据库，合成 Admin 实际为 admin/active；Go overlay 仅将测试监听限制到 `127.0.0.1`，前端预览代理到隔离测试后端，不修改业务认证及正在运行的 Docker 服务。手机开关卡片 44px、Switch 本体 22px、无横向溢出。该阶段的 Production、更完整设备矩阵、目标检查及客户端实际导入/连接属于人工边界；其中 PT-28-01～PT-28-05 已由用户确认完成，R27-05 额外尺寸/焦点/折叠与其他历史项目仍见 ProdTestList。
...
- **TODO：**
  - [x] 先为字段缺口、错误 `version/path` 证据和新增私钥凭据建立失败测试。
  - [x] 修正四插件 schema、证据元数据与固定敏感路径。
  - [x] 验证不完整草稿可保存、Clash 目标检查会阻断，以及私钥全生命周期。
- **测试与验收命令：**
...
- **验证记录（2026-09-04）：**
  - 失败先行测试复现 v2ray/shadow-tls 字段缺口、错误 `version/path` 正式声明、Mihomo 证据缺失，以及插件私钥明文回显且摘要缺失；实现后全部转绿。
  - `cd backend && go test ./internal/ssplugin ./internal/node ./internal/server -count=1`：通过，包含真实协议接口字段与敏感路径断言。
  - `cd backend && go test -race ./internal/node -count=1`：通过。
  - `cd backend && go test ./... -count=1 && go build ./... && go vet ./...`：全部通过。
- **本 Step 边界：** 未修改 URI 编解码、Clash 结构化投影、正式装配诊断门槛或未知插件前端编辑器；旧 `version/path` 等未声明键仅保留数据，固定版本“未消费/未验证”诊断与正式装配一致性仍由 Step 12 落地。未新增数据库 migration，未改写历史节点。
...
- **文档同步：**
  - `Issue14.md`：承接 Step 14 的工程性未完成项、新错误和证据缺口；Issue13 仅保留 R27 历史结论与迁移指针。
...
- **验收标准：** 全部命令通过；固定版本正反例、真实 API 与浏览器核心流程有可复查记录；工作树只包含本轮精确范围内变更；用户人工项目及其结果以 [ProdTestList.md](../../../ProdTestList.md) 为准，未执行时明确标记待办，不宣称交付闭环。

- **本轮执行记录（2026-09-08，工程证据尚未全部收口）：** 后端全量测试、指定竞态测试、`go build ./...`、`go vet ./...`、前端 41 文件/209 用例与生产构建均通过；四类装配器、URI 导入（2 ok/1 skip）、覆盖层及 v2 导出/导入在临时修正执行流中通过。固定 Mihomo 的此前定向执行记录曾在显式 `MIHOMO_11929_BIN` 环境通过，但本次全量核验发现未设置该变量时测试会 `SKIP`，该证据门禁缺口转 [Issue14.md](../Issue/Issue14.md) R28-04。真实 API 浏览器流程覆盖 Setup、管理员注册、四个已知 SS 插件/未知插件切换、未知 `password`/`token` 普通参数、目标检查、保存重开、敏感凭据“已保存（留空保留）”和桌面/375px 无横向溢出；动态插件输入操作捕获两条 Ant Design 输入组件异常，工程问题转 Issue14 R28-01。正式 smoke 夹具问题转 Issue14 R28-02/R28-03。
- **R28-01 修复记录（2026-09-09）：** 失败优先的父子闭环回归确认未知插件参数名每次改名都会改变 Map 行 Vue `key`，使旧 Ant Design Input 在回调完成前卸载。`ProtocolFieldEditor` 改为维护与参数名解耦的本地稳定行身份，新增、改名、父级/外部回填、删除和分支清空时同步对齐，不改变即时模型回写、校验、API 或保存结构。组件宿主和 `NodesView` 回归均覆盖逐字符输入、DOM/焦点保持及最终原子改名；定向 2 文件/53 用例、前端全量 41 文件/210 用例、生产构建、后端全量测试、编译和 vet 通过。PT-28-01～PT-28-05 已由用户确认完成；R28-04 仍待处理。
...
- **R28-04 修复前交叉核验结论（2026-09-09）：** Step 14 的人工项目已全部完成且无问题；R28-01～R28-03 的工程问题和正式 Production smoke 已完成。当时唯一阻挡 Step 14 的问题是 [Issue14.md](../Issue/Issue14.md) R28-04：固定 Mihomo 1.19.29 四个正例在显式 `MIHOMO_11929_BIN` 下已通过，但未设置该变量时测试会静默 SKIP，仍需补强“缺少二进制即失败”的验收门禁；该阶段结论已由下一条收口记录更新。
...
| v1.16 | 2026-09-09 | 完成 Issue14 R28-01 工程修复：动态 Map 行使用与可编辑参数名解耦的本地稳定身份，消除参数名逐字符输入时的 Ant Design 卸载回调异常并保持 DOM/焦点；新增组件宿主与 NodesView 父级即时回写回归，全量前后端验证通过。Step 14 仍受 R28-02～R28-04 和 ProdTestList 人工项目约束。 |
| v1.17 | 2026-09-09 | 完成 Issue14 R28-02 工程修复：Production smoke 应急正常态改为严格 JSON 布尔类型和值断言，避免正常 `false` 误失败及字符串 `"false"` 假绿；失败矩阵与全量构建验证通过。Step 14 仍受 R28-03～R28-04 和 ProdTestList 人工项目约束。 |
