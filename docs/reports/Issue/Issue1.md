...
> 设计记录见 [Design1.md](../Design/Design1.md)；构建方案见 [Build1.md](../Build/Build1.md)、[Build2.md](../Build/Build2.md)（已归档）、[Build3.md](../Build/Build3.md)；编码指令见 [AGENTS.md](../../../AGENTS.md)（**唯一强要求**）。

---

## 一、进行中问题

### R11-04 规则创建「在线编辑」报"未接收到文件"（同 R11-01 根因，深入核验发现）

- **现象：** 规则管理「创建规则」弹窗的在线编辑模式提交必失败，报「未接收到文件」；文件上传模式正常。
- **根因：** `RulesView.doCreate` 文本分支把 `text` 塞入 FormData 且未带 `?mode=text`，后端 `rule.go create` 按查询参数区分文件/文本双模式，误走文件分支 → `FormFile("file")` 失败——与 R11-01 分享订阅完全同款缺陷（后端 5 处双模式端点中版本/自定义已正确，分享/规则两处前端提交格式错误）。
- **影响范围：** 规则在线编辑创建不可用（API 正常）；四类资源「文件/文本」创建链路唯一残余错误点。
- **修复方案（已实施）：** ① `api/rule.ts createRule` 改双模式（FormData 文件 / JSON 文本 + `?mode=text`，对齐 createShare 模式）；② `RulesView.doCreate` 文本分支改 JSON body 提交 `{name, client_type, schemes, text}`。
- **状态：** ✅ 已修复（2026-08-14；验收：`npm run build` + `vitest` 22/22；E2E 冒烟——修复后格式创建成功 `rule-d7xrd65e` 版本 1，旧格式精确复现「未接收到文件」）

### R11-03 自定义订阅上传弹窗按钮错位 + 「粘贴文本」文案不当（UI 体验）

- **现象：** 用户管理「上传自定义订阅」弹窗中「上传文件」「粘贴文本」「选择文件」三个按钮视觉错位（`Radio.Group button-style="solid"` 与普通 Button 混排），且文案「粘贴文本」与版本管理/分享订阅创建弹窗的「在线编辑」不一致。
- **根因：** 该弹窗使用 `Radio.Group`（上传文件/粘贴文本）+ 下方 `Upload` 内「选择文件」按钮的三控件布局，与其他创建弹窗的 Tabs 页签模式不统一。
- **影响范围：** 仅视觉与文案一致性（功能正常）；四类资源创建弹窗交互形态不一致。
- **修复方案（已实施，用户确认 Tabs 方案）：** 改为 `Tabs` 双页签——「文件上传」（页签内含「选择文件（≤50MB）」）/「在线编辑」（TextArea），与版本管理、分享订阅创建弹窗完全统一；移除 `Radio.Group` 切换（Radio 仍保留给设置/重置密码弹窗使用）。
- **状态：** ✅ 已修复（2026-08-14；验收：`npm run build` + `vitest` 22/22；文案统一「在线编辑」）

### R11-02 版本管理「预览」点击后无反馈（UI 体验）

- **现象：** 各类版本管理（订阅/分享/规则/自定义）点击「预览」后弹窗延迟出现，期间无任何加载反馈（内容较大或网络慢时感觉"未响应"）。
- **根因：** 预览弹窗绑定 `:open="previewContent !== null"`——内容要等 `api.preview` 返回后才赋值，加载完成前弹窗不打开，`Modal :loading` 属性在弹窗未打开时不起作用；与编辑弹窗「先开弹窗 + Spin 占位」的模式不一致。
- **影响范围：** 四类资源版本预览交互反馈缺失。
- **修复方案（已实施）：** 独立 `previewOpen` state，点击立即打开弹窗显示居中 Spin「加载内容中…」，内容返回后一次性渲染；加载失败自动关闭并提示（与编辑弹窗加载样式一致）。
- **状态：** ✅ 已修复（2026-08-14；验收：`npm run build` + `vitest` 22/22；加载中 Spin 占位、完成后渲染 pre 内容）

### R11-01 分享订阅「在线编辑」创建报"未接收到文件"（功能缺陷）

- **现象：** 分享订阅「创建分享」弹窗切到在线编辑页签填写内容点「创建」，必报「未接收到文件」；文件上传模式正常。
- **根因：** `SharesView.doCreate` 文本分支把 `text` 塞入 FormData 且未带 `?mode=text`，后端 `share.go create` 按查询参数区分文件/文本双模式，误走文件分支 → `FormFile("file")` 失败。
- **影响范围：** 分享订阅在线编辑创建不可用（API 正常，版本管理/自定义订阅文本创建正常）。
- **修复方案（已实施）：** ① `api/share.ts createShare` 改双模式（FormData 文件 / JSON 文本 + `?mode=text`）；② `SharesView.doCreate` 文本分支改 JSON body 提交 `{name, text}`。
- **状态：** ✅ 已修复（2026-08-14；验收：`npm run build` + `vitest` 22/22；E2E 冒烟——修复后格式创建成功 `share-h4gsf793` 版本 1 + Token 生成，旧格式精确复现「未接收到文件」）
...
5. 非问题的优化候选 / 已知遗留事项归 [Design1.md](../Design/Design1.md) 记录，不记录在本文件。
...
| v1.21 | 2026-08-14 | 追加 R11-01~04：①分享订阅「在线编辑」创建报"未接收到文件"（文本模式 FormData 提交缺 ?mode=text → 改 createShare 双模式 + JSON 提交）；②版本管理「预览」点击无反馈（弹窗内容返回后才打开 → 独立 previewOpen + Spin 占位）；③自定义订阅上传弹窗按钮错位 +「粘贴文本」文案（Radio 改 Tabs 页签统一 + 文案「在线编辑」）；④规则创建「在线编辑」同款"未接收到文件"（深入核验发现，createRule 双模式 + JSON 提交）。验收：`npm run build` + `vitest` 22/22；分享/规则双端点 E2E 冒烟新格式成功、旧格式复现错误；本会话功能改动（多安装包/多下载链接、按版本在线编辑、YAML 检测移除、版本页移动端双态、管理员主页订阅平铺等）已同步 Design1.md/Design1-UI.md |
