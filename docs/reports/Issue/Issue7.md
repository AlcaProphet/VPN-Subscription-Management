...
> 设计记录见 [Design2.md](../Design/Design2.md) 与 [Design2-UI.md](../Design/Design2-UI.md)；构建方案见 [Build8.md](../Build/Build8.md)（当前构建轮次）；编码指令见 [AGENTS.md](../../../AGENTS.md)（**唯一强要求**）。

---

## 〇、本轮说明

- **核查方式：** 已按用户要求完成影响确认（2026-08-27），确认后开始实施；修复后已执行后端 `go test ./...` 与前端 `npm run build`。
- **当前状态：** R22-01 ~ R22-08 **均已修复并验证通过**；R22-03 已由 Build9 Step 1 按“条件化目标选择卡片”方案完成闭环。

---

## 〇.1、修复方案整合（2026-08-26 用户确认；2026-08-27 已实施；R22-03 后补修订）

> 本节汇总 R22-01 ~ R22-08 的最终修复方案、用户决策与主要文件落点；均已实施并验证通过，R22-03 于 Build9 Step 1 完成。

### 已确认决策

- **修复范围：** 8 条问题全部一起修复。
- **R22-03（后补修订）：** 不再无条件展示顶部“目标选择”卡片。若用户仅存在项目原生默认平台（无自定义平台），隐藏目标选择区并自动选中匹配该类型的默认平台；若用户添加了自定义平台，才展示目标平台选择器，并默认选中一个匹配该类型且已有订阅的平台。复用 R22-04 的 `is_default` 字段区分原生/自定义平台。
- **R22-04：** 默认平台（`is_default=1`）的 `product_type` 在后端被修改时**返回 400 拒绝**，前端编辑页禁用产物格式选择。
- **R22-05：** 除查询侧返回显示名称外，**同时补齐成功下载日志中的平台标识写入**，保证平台列可显示名称。
- **R22-06：** 高级模式关闭时不仅隐藏 Xray 节点板块，还**从本次装配表单中剔除 Xray 节点及相关组排序引用**；重编含 Xray 旧蓝图时按用户确认改为**阻止并提示先开启高级模式**。
- **R22-08：** SR-conf 允许不预建规则实体，装配器输入规则名称，生成时自动创建规则并继续版本装配；同时保留“选择已有规则并追加版本”入口（用户确认）。
...
### R22-03 去除“类型与目标”步骤（后补修订：顶部目标选择卡片按自定义平台条件展示）

- `frontend/src/views/admin/AssemblyView.vue`
  - 所有 `stepDefs` 移除 `target`，删除 `skipTargetStep`；
  - **后补修订**：不再无条件渲染 `<Card title="目标选择">`；
  - 若 `platforms` 中不存在 `is_default=false` 的自定义平台，则隐藏目标选择卡片，并自动选中匹配当前副 Tab 类型的原生默认平台；
  - 若存在自定义平台，则保留目标平台选择器，并默认选中匹配当前类型且已有订阅条目的平台。
- `frontend/src/views/admin/assembly/AssemblerShell.vue`
  - 移除 target 步骤标题和 `#target` 插槽（已保留）。
- `frontend/src/views/admin/assembly/TypeTargetStep.vue`
  - 已改造为顶部紧凑目标选择组件；后补修订中仅在有自定义平台时展示（SR-conf 规则名称/已有规则选择部分除外）。
...
- **状态：** 已修复（2026-08-27），已按用户确认“失败时允许部分新条目留存”实施。
...
- **状态：** 已修复（2026-08-27），后端/前端编译与测试通过。
...
- **修复方案（推荐，用户已确认方向；2026-08-27 后补修订）：**
  1. **移除步骤条中的 `target` 步骤**：四类 `stepDefs` 都不再包含 `{ key: 'target', title: '类型与目标' }`。
  2. **首次实施**：将目标选择移到步骤条上方常驻独立选择区。
  3. **后补修订（按用户附图反馈，重新打开）**：不再无条件展示顶部“目标选择”卡片，改为条件式展示：
     - 若 `platforms` 中没有 `is_default=false` 的自定义平台（即只有项目原生默认平台）：
       - **隐藏**目标选择卡片；
       - 自动选中匹配当前副 Tab 类型的原生默认平台；
       - 目标区不重复出现。
     - 若用户已添加自定义平台：
       - **展示**目标平台选择器（不展示“类型”，类型仍由副 Tab 决定）；
       - 默认选中一个匹配当前类型、且已有订阅条目的平台。
     - 复用 R22-04 新增的 `platforms.is_default` 字段区分原生/自定义平台。
     - SR-conf 不受平台选择卡片影响；其规则名称/已有规则入口按 R22-08 保留。
  4. **同步调整 `AssemblerShell`**：移除 `target` 对应步骤标题与渲染槽位。
  5. **清理 `skipTargetStep` 逻辑**：路由 `platform_id`/`rule_id` 参数改为直接预填目标或默认目标，不再用于过滤步骤。
  6. **保留生成前校验**：`targetReady()` 仍应在生成前校验目标已选。
  7. **同步文档**：更新 `Design2-UI.md` §5.3.0/§5.3.4，改为“副 Tab 选择类型；仅存在自定义平台时展示目标平台选择器；仅原生平台时自动匹配并隐藏”。

- **状态：** ✅ 已修复（2026-08-27，Build9 Step 1 实施：顶部“目标选择”卡片改为仅 SR-conf 或存在自定义平台时条件展示；仅原生默认平台时自动匹配并隐藏；前后端测试通过）。
...
- **状态：** 已修复（2026-08-27），后端/前端编译与测试通过；高级模式关闭时重编含 Xray 旧蓝图按用户确认改为阻止并提示。
...
- **状态：** 已修复（2026-08-27），后端/前端编译与测试通过；按用户确认保留“选择已有规则/新建规则”两种方式。
...
| v1.10 | 2026-08-27 | 完成 R22-01 ~ R22-08 全部修复并验证：后端 `go test ./...`、前端 `npm run build` 通过；补充版本 id 断言、平台默认锁测试、SR-conf 自动建规则测试；同步更新各问题状态为已修复。用户补充确认：R22-01 失败允许部分新条目留存、R22-05 分享/规则平台列留空、R22-06 重编含 Xray 旧蓝图时阻止并提示、R22-08 保留选择已有规则入口 |
| v1.11 | 2026-08-27 | 用户附图反馈 R22-03 未完全解决：顶部常驻“目标选择”卡片仍冗余。重新打开 R22-03 并记录后补修订：仅当存在自定义平台时才展示目标平台选择器；仅原生默认平台时隐藏该区域并自动匹配对应默认平台；复用 `platforms.is_default` 区分。本次仅更新 Issue7，未改代码 |
