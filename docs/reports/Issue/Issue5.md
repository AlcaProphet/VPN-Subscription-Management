...
> **文档定位：** 本文档是 VPN 订阅管理系统的**当前问题记录**（记录错误与修复方案，非强制，经验参考），承接已归档的 [Issue4.md](Issue4.md)（R19 系列，保留备查）。
...
- **状态：** ✅ 已修复（2026-08-24，后端装配 context 增加订阅摘要，前端预检使用该数据；前后端测试通过）
...
- **状态：** ✅ 已修复（2026-08-24，30 分钟整体超时 + 取消端点 + 前端取消按钮已落地；后端/前端测试通过）
...
- **状态：** ✅ 已修复（2026-08-24，按已确认决策：编辑凭据变更后对所有保留目标 Remove+Add 重推）
...
- **状态：** ✅ 已修复（2026-08-24，路由按“未配置匿名/已配置管理员”收敛，新增 HTTPS + 公网 IP 校验、重定向限制、限流，且不再回显上游响应体；后端/前端测试通过）
...
| v2.0 | 2026-08-22 | 用户决策已确认（R20-08 超时+取消、R20-11 转人工、R20-15/25 文档统一回写、R20-27 凭据重推、R20-26 重复项 400、R20-09 完整测试、R20-10 一键 Production smoke、R20-19 严格导入保护、先写 Build8）。已创建 [Build8.md](../Build/Build8.md) 并完成 Step1~4 代码/前端/文档/测试；后端 build/vet/test 全绿，前端 65 例单测通过。 |
