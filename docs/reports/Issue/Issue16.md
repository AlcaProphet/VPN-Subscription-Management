...
> **归档说明：** R30-01～R30-06 已完成工程修复、自动化验证及用户确认的人工核验，Issue16 已于 2026-09-14 按归档规则移入 `docs/reports/Issue/`。R30-05 的真实 OIDC 登录及其后续遗留问题已独立由 [Issue17.md](../../../Issue17.md) 跟踪，不阻塞本 Issue 归档。
...
- **核验后残余/既有问题：** 提供商切换非 Secret 字段混合、字面/不可解密占位符识别、signing_key JSON 导出损坏、真实 OIDC endpoint HTTPS 校验等见 [Issue17.md](../../../Issue17.md)，不并入 R30-05 本轮阻塞修复。
- **验收边界：** 使用隔离 mock 覆盖 GET/保存不改密文/显式替换/占位符拒绝、提供商切换、测试接口回退、登录所用 Secret 解密与导入校验；真实 OIDC 登录只在用户提供隔离环境后单独核验。不得把静态路径分析表述成已复现的现场问题。
- **状态：** ✅ 工程修复（含两轮核验发现的阻塞/边界项）、自动化验证及本 Issue 范围内人工核验完成；真实 OIDC 登录与 Issue17 遗留项继续单独处理，不属于本 Issue 的未完成项。
...
4. R30-05 工程修复及本 Issue 范围内人工验收已完成；真实 OIDC 登录、原 R30-07「暂未启用」未落库问题及其他遗留项继续由 [Issue17.md](../../../Issue17.md) 独立跟踪，现编号为 R31-07。
