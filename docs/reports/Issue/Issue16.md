# Issue16.md — VPN 订阅管理系统邮件测试与 SMTP 优化（已归档）

> **归档说明：** R30-01～R30-06 已完成工程修复、自动化验证及用户确认的人工核验，Issue16 已于 2026-09-14 按归档规则移入 `docs/reports/Issue/`。R30-05 的真实 OIDC 登录及其后续遗留问题已独立由已归档 [Issue17.md](Issue17.md) 跟踪，不阻塞本 Issue 归档。
...
- **人工验收结果（2026-09-14）：** 用户确认密码重置邮件的启用范围、触发、到达、`valid / missing / used / expired` 四态及表单行为均已人工测试完成，已使用或过期链接未渲染密码表单。
- **状态：** ✅ 工程修复、自动化验证及人工核验完成；Mailgun 真实账号测试不属于当前合同，未作为本 Issue 的阻塞项。
...
- **状态：** ✅ 工程修复、自动化验证及改动后真实 SMTP 发信人工复验完成。
...
- **状态：** ✅ 通用 SMTP 工程优化、自动化验证及改动后真实服务人工复验完成。
...
- **修复实施（2026-09-14 用户单独授权）：**
  ...
  - OIDC 测试接口：管理员面板仅在 `provider_type/base_url/realm/client_id` 与已保存配置完全一致且 Secret 为空时回退已存明文；Setup 匿名入口不回退；无可用 Secret 时返回“未执行凭据校验”警告，不再静默报全通过；
  - 配置导入在本地登录关闭时，用导入包签名密钥解密校验占位符/不可解密密文，避免导入后认证死锁；
  - 核验后阻塞修复：`SaveLocalAuth` 改用实际 OIDC 可用性校验（参数可读、`SecretUsable` 非空且非 `***`；mock 仅 Dev 模式可用），损坏/空/占位符 Secret 时禁止关闭本地登录；`TestConnection` 回退绑定已保存目标，避免把 Secret 发往任意请求 URL；`currentParams`/`Exchange` 拒绝损坏占位符进入登录链路；并补充 BaseURL/Realm 不一致、已存 `***` 与 StartFlow/Exchange 回归测试。
- **核验后残余/既有问题：** 提供商切换非 Secret 字段混合、字面/不可解密占位符识别、signing_key JSON 导出损坏、真实 OIDC endpoint HTTPS 校验等见已归档 [Issue17.md](Issue17.md)，不并入 R30-05 本轮阻塞修复。
- **验收边界：** 使用隔离 mock 覆盖 GET/保存不改密文/显式替换/占位符拒绝、提供商切换、测试接口回退、登录所用 Secret 解密与导入校验；真实 OIDC 登录只在用户提供隔离环境后单独核验。不得把静态路径分析表述成已复现的现场问题。
- **状态：** ✅ 工程修复（含两轮核验发现的阻塞/边界项）、自动化验证及本 Issue 范围内人工核验完成；真实 OIDC 登录与已归档 Issue17 遗留项已在各自范围内闭环，不属于本 Issue 的未完成项。
...
- **状态：** ✅ 代码及自动化修复完成，真实浏览器与实际发信人工验收完成。
...
### 面板中的“自定义发信内容”模块（历史预留，已由 Build28 实现）

> **当前状态（2026-09-15）：** 本预留已由 [docs/reports/Build/Build28.md](../Build/Build28.md) 完成工程实现、自动化与隔离 smoke 并归档。以下内容保留为归档时的研究记录；当前入口、配置键、行为与人工边界以 Build28、代码和 [ProdTestList.md](../../../ProdTestList.md) 为准。

- **放置与状态：** 预留在同一“通知”分组、SMTP 设置与测试区之后的独立“邮件内容”卡片；SMTP 连接参数与邮件文案分别保存，避免只改正文时触碰密码和连接方式。归档时页面、数据库和发送结果均未新增该模块；该模块随后已由 Build28 实现，当前实现见 [docs/reports/Build/Build28.md](../Build/Build28.md)。
...
- **待确认范围（归档时）：** 首版是否包含审批通过/拒绝和欢迎本地/OIDC 的分别编辑，以及是否允许自定义测试邮件。该范围随后已由 Build28 定稿并实现：五分支分别编辑，测试邮件保持固定，不允许自定义。
...
| 未来邮件内容设置与 `backend/internal/mail` | 五个业务文案分支的主题/纯文本正文、变量白名单、预览、默认回退、导入导出及发送上下文；本 Issue 归档时只预留，现已由 Build28 实现，不改 SMTP 连接测试邮件。 |
...
| 2. SMTP 发送与诊断 | 强制 STARTTLS、直连 TLS、本地无认证明文、阶段化安全错误、总超时。 | 协议 mock 与错误脱敏回归通过；用户确认改动后真实 SMTP 发信人工复验完成。 |
| 3. 表单与用户管理 | 三态与认证开关、未配置提示、测试回执、R30-06 邮件入口状态。 | 前端定向测试、构建及浏览器人工检查通过。 |
| 4. 联合门禁与真实验收 | 后端 build/vet/test、前端 build/test；隔离环境按实际提供的 SMTP 服务商逐项核验。 | 自动化通过；用户确认隔离 Production、真实 SMTP、收件箱及 Build11 重置链接四态人工核验完成。 |
| 独立问题 R30-05 | OIDC Secret 空回显/状态、空值保持、显式替换、占位符拒绝、加载时解密、测试连接回退与导入占位符校验；不并入 SMTP 连接方式或邮件内容修改。 | 后端 build/vet/test、errgate 与前端 build/test 通过；真实 OIDC 登录由 Issue17 继续单独核验，不阻塞本 Issue 归档。 |
...
3. R30-01 的重置链接四态、人工收件结果及 R30-02～R30-04 改动后真实 SMTP/浏览器复验已由用户确认完成；对应人工项已从 [ProdTestList.md](../../../ProdTestList.md) 移除。
4. R30-05 工程修复及本 Issue 范围内人工验收已完成；真实 OIDC 登录、原 R30-07「暂未启用」未落库问题及其他遗留项已由已归档 [Issue17.md](Issue17.md) 独立跟踪，现编号为 R31-07。
5. 本 Issue 归档时自定义邮件内容仅作模块预留；2026-09-15 已由 Build28 完成五分支邮件内容编辑/预览、发送接线、配置导入导出与备份快照覆盖，测试邮件保持固定。
...
| v1.9 | 2026-09-14 | 按用户单独授权修复 R30-05：OIDC Secret 空回显与只读状态、空值保持/显式替换/占位符拒绝、加载时解密、测试连接回退与导入占位符校验；后端 build/vet/test、errgate 与前端 build/test 通过；另登记 R30-07「暂未启用」未落库相邻问题（后迁入 Issue17 R31-07），真实 OIDC 登录待隔离环境核验。 |
| v1.10 | 2026-09-14 | 核验 R30-05 后修复 3 个阻塞项：损坏 Secret 禁止关闭本地登录、TestConnection 回退绑定已保存目标且 Setup 不回退、currentParams/Exchange 拒绝损坏占位符；新增 Issue17 登记 signing_key 导出损坏、真实 OIDC endpoint HTTPS 校验、提供商切换字段混合等遗留问题；门禁重新通过。 |
| v1.11 | 2026-09-14 | 二轮核验补齐防死锁边界：mock 仅 Dev 可用、空 Secret 视为未配置，禁止据此关闭本地登录；TestConnection 回归测试补齐 BaseURL/Realm 不一致、已存 `***` 与 StartFlow/Exchange 场景；另登记 mock 模式校验遗留问题 R31-06。 |
| v1.12 | 2026-09-14 | 按用户指令将原 R30-07「暂未启用」未落库问题迁入 Issue17，重编号为 R31-07。 |
| v1.13 | 2026-09-14 | 用户确认 Issue16 范围内所有人工核验项目已完成；补录 R30-01～R30-06 的人工验收结论，移除未完成表述，并按归档规则将 Issue16 移入 `docs/reports/Issue/`；Issue17 遗留项继续独立跟踪。 |
