...
- **当前状态：** Build21 Step 14 的 PT-28-01～PT-28-05 人工项目已由用户确认完成且未发现问题，但 Step 14 仍受 R28-04 固定版本证据门禁及工程收口约束；BuildReport4 中仍未闭环的工程问题（D3-1～D3-10、N-core-1～6、N-node-6、安全 N01～N07 等）已按来源归入本文件。剩余历史人工项目见 [ProdTestList.md](ProdTestList.md)，不在本文件重复登记为缺陷。
...
- **剩余收口：** R28-04 固定 Mihomo 证据门禁及本文件其他工程问题仍按步骤一和后续步骤处理，不因人工项目完成而自动关闭。
...
- **当前核对证据（2026-09-09）：** `/Applications/Clash Verge.app/Contents/MacOS/verge-mihomo` 报告 `Mihomo Meta v1.19.29 darwin arm64`；显式设置 `MIHOMO_11929_BIN` 后 `obfs`、`v2ray-plugin`、`shadow-tls`、`restls` 四个正例均通过。未设置该变量的同一测试仍为 `SKIP`，所以固定版本内容本身已有通过证据，但“缺少二进制不得静默通过”的验收门禁仍未完成。
- **状态：** ☐ 待补强验收门禁（固定版本正例已显式执行通过）
...
- **步骤二关闭：** PT-28-01～PT-28-05 已由用户确认完成且未发现问题，相关人工项目已从 ProdTestList 当前待办移除；Build21 Step 14 仍需步骤一的 R28-04 证据门禁和其他工程收口。
