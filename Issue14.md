...
- **分项修复方案：**
  ...
  8. **R28-07H（遗留 Tailwind gray/white 类）：** 当前确认剩余 5 处硬编码颜色，位于 `EditableCombobox.vue` 的输入禁用态/下拉背景/悬停态/空态和 `NodeCheckPanel.vue` 的预览背景。逐项替换为项目已有语义 Token，确保浅色/深色主题均保持层级、边框、悬停和禁用可读性；增加前端静态扫描，阻止受管源码重新引入 `gray-*`/`white` 等已禁用颜色类，并补两主题组件快照或 DOM 类断言。
