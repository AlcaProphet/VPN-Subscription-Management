...
- [ ] R31-04：在 R31-03 目标读取最小损坏/重填语义之上，补齐当前提供商 GET、测试连接与 `signing_key` 故障的完整损坏状态机；合同已按第三轮核验更新。
- [ ] R31-06：统一限制 OIDC mock 仅 Dev 可用；现有 Production 配置导入遇任何 mock 配置整体拒绝，并补保存、状态、登录及导入回归。未来整站恢复边界见 [Design5.md](Design5.md)，单独实施。
- [ ] R31-07：明确「暂未启用」是本地 UI 状态还是持久化停用语义；若为后者，设计停用保留参数与重新启用合同；同时深入研究 R31-03 风险 4 的 `SaveOidc` / `ClearOidc` 多键写入事务边界与故障注入证据。
