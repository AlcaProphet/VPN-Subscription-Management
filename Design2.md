...
| 24 | **配置导入导出** | **xray_instances 数据纳入配置导入导出**：export.go 扩展导出实例连接数据，导出格式升级 format_version=2；**导入语义：整体覆盖 xray_instances 表**——事务内删除既有实例行（xray_users / nodes 随 FK ON DELETE CASCADE 级联清理，见 5.9），按导出内容重建实例行，导入完成提示执行节点检测刷新与账号对账补推；traffic_records 等不引用实例的表不受影响；format_version=2 payload 新增 instances 数组字段（见 5.9，审核 F3-6/R3） |
...
| `xray_instances` | id, **name（UNIQUE）**, **slug（UNIQUE，`instance-` 前缀短码；节点名 {实例slug}-{入站tag} 的实例 slug 来源，审核 F4-6）**, api_addr, api_tag, enabled, **last_collect_at / collect_status / collect_error**（采集状态与告警，见 5.8） | 实例（1-5 台），api_addr 为 TCP 地址（IP 白名单防护）；**被 xray_users.instance_id 与 nodes.instance_id 外键引用（ON DELETE CASCADE，见下行与 xray_users 行，审核 R3）**；**纳入配置导入导出**（决策 #24，format_version=2；**导入语义：整体覆盖 xray_instances 表**——事务内删除既有实例行（xray_users / nodes 随 FK 级联清理），按导出内容重建，导入完成提示执行节点检测刷新与账号对账补推；traffic_records 等不引用实例的表不受影响，审核 D10/F3-6/R3；**导入保护：向已配置系统导入时若 signing_key 将变化且存在任一业务密文（users.uuid_encrypted / users.proxy_secret_encrypted / nodes.protocol_json 凭据字段），拒绝导入并提示「配置导入仅适用全新部署/同密钥往返，在用实例请使用备份恢复」，审核 A4 决策**） |
...
| `platforms`（改） | + product_type（yaml/subs，默认 yaml） | 平台承载的订阅产物格式（装配时按此过滤目标平台，审核 R1）；**模板 product_type 须与平台 product_type 一致**（创建/上传/装配时校验） |
| **1009 迁移清理口径（审核 A3/D1/R2 决策）** | 显式 SQL 按序执行 + 框架钩子 | ①`DELETE FROM download_tokens WHERE subscription_id IS NOT NULL`（显式订阅 Token 不再新发，存量一并清；无标识/自定义 Token 保留）；②`DELETE FROM versions WHERE owner_type='subscription'`；③删 group_selections / subscription_group_rel；④**以「新表 + rename」切换重建 subscriptions**（含 UNIQUE(platform_id) 与 product_type）：**先创建新 subscriptions 表** → **重建 download_tokens**（保留 subscription_id 列，外键重新指向新 subscriptions 表，保留 ON DELETE CASCADE 级联语义）→ **DROP 旧 subscriptions 表**（此时无表引用它，可安全 DROP，避免 foreign_keys=ON 下 DROP 被外键定义拒绝）→ **rename 新表为 subscriptions**（download_tokens 外键定义随 rename 自动跟随指向新名；审核 D1/R2）；⑤**迁移框架新增「迁移后钩子注册表」机制**：注册 1009 钩子删除 contents/subscription/ 目录（幂等——目录不存在即跳过；失败记日志不阻断启动，审核 F1-3） |
