...
- **用量响应头（决策 #23）**：下载响应注入 `subscription-userinfo: upload=; download=; total=; expire=4102444800`（远未来时间戳表达无到期，不用 0 避免客户端误解为已过期）+ `profile-update-interval` + `profile-web-page-url`，与平台附加头机制融合；超限用户 total 仍返回配额值
