...
R27-08（2026-09-04）为目标检查面板补充运行时响应兼容：诊断长度判断与排序统一通过 `diagnostics ?? []` 读取，旧服务端或异常链路返回 `null` 时仍显示“未发现诊断”，不再中断整个检查面板。混合响应与既有迟到响应测试通过；前端全量 41 个测试文件／195 个用例及生产构建通过，保留既有大 chunk 警告。
...
| v1.9 | 2026-09-04 | R27-08 对齐修复：目标检查面板集中归一化运行时 `diagnostics: null`，长度判断和排序共用安全空数组；补充无诊断与多目标混合响应回归，既有迟到响应测试继续通过。前端全量 41 文件 / 195 用例与生产构建通过。 |
