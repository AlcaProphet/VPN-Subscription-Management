...
R27-08（2026-09-04）收紧目标检查响应契约：每个 `TargetCheckResult.Diagnostics` 始终为非 nil 数组，成功无诊断时 JSON 固定输出 `[]`；状态计算、预览、脱敏和不落库语义不变。service、多目标正反例与真实 HTTP route 回归均断言空诊断不是 `null`。后端全量测试、编译和 vet 通过。
...
| v1.4 | 2026-09-04 | R27-08 对齐修复：目标检查成功无诊断结果统一序列化为 `diagnostics: []`；新增 service、多目标混合和真实 HTTP 响应契约回归，后端全量测试、编译与 vet 通过。 |
