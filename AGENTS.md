...
- 可访问管理面板：管理用户、订阅、平台、规则、面板配置（含 OIDC、速率限制、公告栏）、查看日志
...
**面板配置**:
- 查看和修改 OIDC 提供商参数（含测试连接、切换提供商类型）
- 查看和修改速率限制参数（登录/下载 API 限流）
- 编辑系统公告栏（首页顶部展示）
- **调试模式开关**：开启后 5xx 错误响应返回详细内部错误信息以便排查（生产环境默认关闭）。开关状态持久化在 `system_config.debug_mode`，服务重启后保留
...
**所有下载端点统一行为**: 无论通过何种途径访问下载链接，后端始终以 `Content-Type: text/plain; charset=utf-8` 输出订阅配置的纯文本内容（类似 GitHub RAW 输出）。VPN 客户端可直接读取该纯文本响应。

**Clash Verge 兼容例外**: 当下载平台为 `clash-verge` 时，后端额外返回以下专用响应头以兼容该客户端的订阅解析需求：
- `Content-Disposition: attachment; filename*=UTF-8''...yaml` — 客户端依赖此头识别订阅文件名称
- `profile-update-interval: 300` — 建议客户端每 300 分钟自动更新
- `profile-web-page-url: {frontend_url}` — 用户可在客户端内跳转到管理页面
此行为不影响其他平台（v2rayNG、Shadowrocket 等），它们仅获得纯文本响应。
...
- 不可硬编码密钥，使用自动生成后存 SQLite（业务配置如 OIDC/密钥/速率限制一律通过 Web UI 存 SQLite，不用 .env）。仅监听端口等运维参数可选通过环境变量覆盖（如 PORT 默认 8080、LOG_LEVEL 默认 info、LOG_FORMAT 默认 console）
...
- `GET /rules` — 规则列表（不含下载 Token）。需 JWT 认证，返回规则的元数据（id、name、client_type、versions），供前端 `/rules` 页面展示。登录用户均可访问
- `GET /rules/:id/download-link` — 获取指定规则的下载链接。需 JWT 认证，返回 `{ url: "/api/v1/rules/:id/download?token=xxx", rule_name: "..." }`，前端据此展示下载链接供用户复制
- `GET /rules/:id/download?token=` — 规则下载（?token= 验证 rule_tokens，无需认证，速率限制），供 VPN 客户端直接使用
...
- `GET /rules` — 规则列表（需 JWT，不含 Token）
- `GET /rules/:id/download-link` — 获取指定规则的下载链接（需 JWT）
...
- 系统配置: `GET /admin/oidc-config`、`POST /admin/test-oidc`、`POST /admin/system/configure`、`POST /admin/system/switch-provider`、`GET/PUT /admin/system/rate-limit`、`GET/PUT /admin/system/debug-mode`（调试模式开关，可扩展更多 debug 功能）
...
单容器部署，端口绑定 `127.0.0.1`。可选环境变量用于控制日志输出：
...
services:
  app:
    ...
    # 可选环境变量（取消注释后 docker compose up -d 生效）:
    # environment:
    #   - LOG_LEVEL=debug           # debug | info（默认） | warn
    #   - LOG_FORMAT=json           # 不设 = 彩色控制台, json = 结构化 JSON
