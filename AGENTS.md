...
**用户（需 JWT Bearer）**:
- `GET /user/platforms` — 平台列表（返回 id、name、description、client_schemes、download_url + 当前用户的 download_token，首页使用此端点）
- `GET /user/update-time` — 首页更新时间戳（所有订阅当前版本 updated_at 的最大值）
- `POST /user/refresh-token` — 刷新指定平台下载 Token。请求体 `{ platform, type }`，type 为 default/advanced（自定义订阅刷新由管理员在用户管理页操作，不走此端点）
...
| 自定义订阅 Token | download_tokens 表保持 user_id + platform + type + custom_sub_id，Token 仍绑定 platform。custom_sub_id 非空时 type 置为 NULL，复用唯一键为 user+platform+custom_sub_id |
...
| 自定义订阅 Token type | custom_sub_id 非空时 type 字段置为 NULL（不参与下载判定），复用唯一键为 user+platform+custom_sub_id，避免多 Token |
| 用户 is_advanced 变更后旧 Token | 升级/降级时后端自动删除该用户所有旧 Token，用户下次访问首页重新生成对应级别 Token（防止旧级别 Token 绕过分级限制） |
| 删除自定义订阅后关联 Token | 级联删除所有 custom_sub_id 指向该订阅的 Token，用户下次访问该平台重新生成默认/高级 Token |
...
| 自定义订阅刷新 Token | 新增 `POST /admin/users/:id/custom-subscription/refresh-token?platform=xxx`，管理员在用户管理页为指定平台的自定义订阅刷新 Token |
| 版本创建端点 | `POST /:resource/versions` 同时支持 multipart 文件上传和 JSON 文本 body（根据 Content-Type 区分），两种方式均创建新版本并切换 current |
| 平台列表端点职责 | `GET /platforms`（公开）返回 name/description/client_schemes/download_url（不含 Token）；`GET /user/platforms`（JWT）返回相同字段 + download_token，首页使用后者 |
| 版本对象 schema | versions JSON 数组每个版本对象含 `{ version: int, file_path: string, created_at: datetime, updated_at: datetime }`，updated_at 在切换 current 或编辑时更新 |
| 自定义订阅下载途径 | 复用 `GET /subscriptions/:platform/download-token?token=`（custom_sub_id 非空时返回自定义内容），无需独立端点 |
| 环境变量范围 | 仅监听端口等运维参数可选通过环境变量覆盖（如 PORT）。业务配置一律通过 Web UI 存 SQLite，不用 .env |
| OIDC 回调 JWT 传递 | 后端 302 到前端中转页 `/auth/callback?token=xxx`，前端提取 JWT 存 localStorage 后 replaceState 清空 URL。避免 token 泄露到 Referer/浏览器历史 |
| 版本上传并发安全 | nextVersion 计算与 versions JSON 更新放在单个 SQLite 事务中，对资源行级锁（UPDATE ... WHERE id=?）。SQLite WAL 模式下可行 |
| current 软链接原子切换 | 切换 current 时用临时 symlink + rename() 原子替换（创建 current.new → rename(current.new, current)），避免下载过程读到半切换状态 |
| 订阅未配置降级 | 高级用户访问未配 advanced 的平台时，卡片显示"高级订阅未配置，请联系管理员"，不生成 Token。管理员首页该平台 advanced 按钮显示"未配置"。不降级到 default |
| Setup 与 admin_initialized | Setup 完成时只置 configured=true，admin_initialized 仍为 false。首个用户登录时触发 admin_initialized=true。防止 users 表清空后再产生新管理员 |
| access_logs 状态字段 | 日志补充 status(success/failed) + error_reason，记录下载是否成功及失败原因（token_invalid/file_not_found 等） |
| 速率限制响应 | 超出限速返回 HTTP 429 + Retry-After header。登录端点附 JSON 错误供前端弹提示；下载端点返回纯文本错误供客户端识别 |
| OIDC Client Secret 存储 | 各提供商独立加密字段（keycloak_client_secret_encrypted / auth0_client_secret_encrypted / generic_client_secret_encrypted），切换提供商时按 provider_type 读取对应字段，保留已填值 |
| 级联删除规则 | 删除用户/订阅/规则/分享/平台时，级联清理关联 Token 和版本文件（详见 5.x 级联删除规则） |
| 订阅未配置对称降级 | default/advanced 任一未配置时，对应级别用户看到"未配置"提示，不生成 Token，不跨级降级/升级 |
