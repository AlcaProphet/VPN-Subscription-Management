...
- **开工前只读核验（阶段 A）：** 复核分支 `beta`、HEAD `8e247ef95338ccfa3bd941c454fd9deaa940b03c`、`beta` 领先 `origin/beta` 2 个提交、工作区干净；确认 Step 17～19 成果确实位于当前 HEAD；由代码计算确认 manual 协议 19 项、显式 adapter 15 项、缺失集合恰为 `ss／vmess／vless／trojan`、`legacyAdapterPendingCount()==4`；固定 tag `/private/tmp/mihomo-v1.19.31`（HEAD `ab405bad…`、`git describe = v1.19.31`）与固定二进制 `/Users/kyle/Desktop/Repo/Temp/mihomo-darwin-arm64-go124-v1.19.31`（`-v` = `Mihomo Meta v1.19.31 darwin arm64 with go1.24.13`、SHA-256 `3ed36cab…`）逐项一致且无 wire shape 冲突；未发现停止条件。
- **用户确认的六项取舍：** ①固定二进制使用上述路径；②预览脱敏改为**构造后**（Clash 字段级、URI 值级含 base64／URL 编码形式与 fail-closed 兜底），诊断与自检仍使用真实参数；③删除 `legacyClashProxy` 与 `clashProxy()` 的静默回退，`RenderPlan.ManualProxies` 改由 `buildClashProxy` 构造并传播错误；④`target_evidence` 采用轻量方案（状态全由 adapter 诊断 + `CheckClashContent` 计算，诊断 `field_path` 统一回指 schema canonical path，不新增能力清单）；⑤`CheckClashContent` 新增门禁全量生效，但只纳入项目明确禁止、Xray 动态节点（仅 vmess／vless／trojan／shadowsocks）不可能产生的结构；⑥URI 链接在构造后按值替换，包含原文／`url.QueryEscape`／`url.PathEscape`／Base64 编码形式。
- **失败优先证据（实现前真实失败）：** 新增 19 个测试函数后先运行，得到的目标能力缺失失败包括：`manual 协议缺少显式 Clash adapter: [ss vmess vless trojan]`、`legacy adapter 数量应为 0，实际 4`、`未注册 adapter 必须返回显式错误而不是 legacy 投影`、四协议 wire 残留 `smux:map[enabled:false]`、`VMess alpn 必须为 YAML 数组`、`h2-opts.host` 未按 `[]string` 输出、`Trojan 内层 SS 未启用时不得进入 wire`、`检查预览泄漏凭据`（四协议均有）、`字段 skip-cert-verify／packet-addr／udp-over-tcp／client-fingerprint 的诊断结论不符合目标表达能力`、`$.proxies[0]*` 自检路径无法回指 schema、15 协议 shape／互斥门禁全部缺失。失败原因均为目标能力未实现，非测试构造或环境问题。
- **四个显式 Clash adapter（`assembly/clash_protocols.go`）：**
  - `ssClashAdapter` → v1.19.31 `ShadowSocksOption`：`password／cipher／udp／plugin／plugin-opts／udp-over-tcp／udp-over-tcp-version／client-fingerprint` ＋ BasicOption 白名单；插件复用既有 `projectSSPluginForClash`（存储对象 → `plugin` + `plugin-opts` 并注入 Clash 目标默认值，随后删除全部存储键）；`smux` 只在 `enabled=true` 时以 proxy 级对象输出。
  - `vmessClashAdapter` → `VmessOption`：点名键复制 ＋ `alpn` 数组 ＋ `ech-opts`（启用复制）／`reality-opts`／`http-opts`／`h2-opts`／`grpc-opts`／`ws-opts`；**始终输出 `alterId`（缺省 0）与 `cipher`（缺省 auto）**，对齐固定 tag 无 `omitempty`；`h2-opts.host` 按 `[]string` 输出；tag 中的 `tlsmirror-opts／mekya-opts／mkcp-opts` 不在白名单内。
  - `vlessClashAdapter` → `VlessOption`：`uuid／flow／tls／udp／packet-addr／xudp／packet-encoding／encryption／network／skip-cert-verify／servername／client-fingerprint` ＋传输对象 `reality-opts／http-opts／h2-opts／grpc-opts／ws-opts／xhttp-opts`；`ws-path／ws-headers` 旧别名已在归一化阶段收敛进 `ws-opts`，不单独进入 wire；schema 未开放的 ECH／伪装／mTLS 字段不输出。
  - `trojanClashAdapter` → `TrojanOption`：`password／sni／udp／network／client-fingerprint` ＋ `alpn` 数组 ＋ `reality-opts／grpc-opts／ws-opts`；内层 `ss-opts` **只在 `enabled=true`** 时输出。
  - 新增 `copyClashALPN`（兼容首批协议历史沿用的逗号分隔 `alpn`，保持旧 legacy 投影语义）、`copyClashTransportObject`（列表子字段按内核数组形状输出）、`copyClashFeatureObject`（按 `enabled` 判定）、`clashAlterID`／`clashVMessCipher`。四者均使用完整 BasicOption 白名单（固定 tag 的四个构造器都消费 `tfo`／`mptcp`）。
- **legacy 路径归零与单一入口：** `buildClashProxy` 对未注册协议返回显式错误；删除 `legacyClashProxy`、`clashProxy()` 与 `legacyAdapterPendingDiagnostic`；新增 `manualClashPlanProxies()` 让渲染计划与生成产物同源，失败即返回 400；`normalizeClashFields` 保留但注释明确只服务 Xray 动态节点（下载重渲染）路径。**`legacyAdapterPendingCount()` 4→0，19 个 manual 协议全部经过显式 adapter。**
- **构造后脱敏（`node/check.go`、`node/sensitive_paths.go`、`assembly/node_check.go`）：** `node.Check` 改为向目标适配器传递**真实活动参数**（仍剥离 `_credential_id` 等内部编辑元数据），`check_id` 继续使用脱敏摘要。新增 `node.RedactSensitiveParams`（预览对照副本）与 `node.SensitiveValues`（按 SensitiveFields 模式收集有效字符串值；列表条目**不要求** `_credential_id`，因此在元数据剥离后的副本上同样有效）。Clash 预览在 `buildClashProxy` 构造完成后按具体敏感值替换字符串叶子；URI 预览在 `RenderLink` 构造完成后用「脱敏副本对照」定位差异区间并替换（`spliceLinkPreview`），再按原文／URL 转义／Base64 形式做值级替换，最后做 fail-closed 校验（残留则回退到对照构造结果，必要时不返回预览）。自检 `CheckClashContent` 仍针对**真实产物**运行。未含活动凭据的配置预览逐字节不变。
- **URI 能力收口（`assembly/node_check.go`）：** 新增 `paramValueActive`／`hasConfiguredObject`／`hasEnabledObject`／`linkDroppedHeaderPath`／`addBasicOptionURIDiagnostics`／`addURIPartialFields`。首批四协议补齐逐字段审计：ss（显式关闭的 `udp`、`udp-over-tcp`、`udp-over-tcp-version`、`client-fingerprint`、`smux`）、vmess（`generic-subs` 不表达 `skip-cert-verify` 而 SR 可表达；`reality-opts` 两个目标都不可表达 → error；`packet-addr／xudp／packet-encoding／global-padding／authenticated-length／smux`；非 Host 请求头）、vless（REALITY 分支的 `skip-cert-verify`、`packet-*`、`smux`、非 Host 请求头）、trojan（`reality-opts` → error、显式关闭的 `udp`、`client-fingerprint`、非 Host 请求头）；公共字段按目标区分 `tfo`（SR 的 vmess／vless 可表达）与其余项。**SR 与 generic 分别判断，不共用笼统结论。** 8 个无映射协议的 `target_unsupported`／`skip`／空预览保持不变。
- **evidence 消费与 canonical field path：** 新增 `node.SchemaPathExists`（支持嵌套对象、固定对象列表与开放 Map），`checkClashNodeTarget` 通过新增的 `canonicalClashIssuePath` 把 `$.proxies[n].<field>` 映射回 schema 规范路径，SS 已知插件的 `plugin-opts.<k>` 映射到其存储对象（例如 `obfs-opts.mode`）；无法回指时不再伪造路径。`CheckClashContent` 的「缺少 server／port」「缺少必填字段」问题路径改为字段级（`$.proxies[n].<field>`），使 issue path 与 evidence path 都可精确定位。诊断 `Evidence` 限定在已登记来源集合内。
- **`CheckClashContent` 新增门禁（`assembly/selfcheck.go`）：** 新增 `checkProtocolWireShape` 分发与 11 个协议检查：Tailscale 禁止顶层 `server／port`；Hysteria2 `port`／`ports` 二选一且混淆子字段依赖 `obfs`；Mieru `port`／`port-range` 二选一；WireGuard `peers` 与顶层 peer 字段互斥、Peer 逐项必填 `server／port／public-key／allowed-ips`、`reserved` 恰 3 字节且 0–255；TUIC v4 Token 与 v5 UUID／密码互斥、UOT 版本依赖开关；TrustTunnel 复用两组互斥；OpenVPN 认证组成对、三种 TLS key 互斥、`key-direction` 依赖 `tls-auth`；AnyTLS 三种伪装对象互斥；ShadowQUIC 拒绝固定 tag 不存在的 TLS 校验／证书／ECH／UOT 版本字段（`udp-over-stream` 是真实字段，保留）；Snell v1／v2 禁 `udp`、v1 禁 `reuse`；MASQUE `network` 只接受 `h2／h3-l4proxy` 且 `h3-l4proxy` 禁 UDP。SS 插件结构门禁保持原有实现。
- **测试与夹具：** `clash_protocols_test.go` 新增 manifest 门禁、未注册协议显式错误、四协议 wire shape、四协议 state_only／内部元数据排除、check 预览与正式装配语义一致、构造后脱敏、渲染计划同源、19 协议无 `legacy_adapter_pending`；`node_check_test.go` 新增 URI 能力矩阵（11／8 与 manual 清单对齐）、8 协议 × 2 目标稳定 skip、逐字段 per-target 诊断、能力驱动状态、canonical field path（含夹具全量扫描与证据来源白名单）、检查零落库（全部业务表行数快照）、自检路径映射回 schema；`selfcheck_test.go` 新增 17 项协议 shape／互斥反例。把 15 个固定内核测试文件的 `clashProxy` 调用统一改为新增的 `clashProxyFixture`（走 `buildClashProxy`），`TestClashProxyKeyOrder` 与 SS 插件投影测试同步改走同一入口；`links_test.go`、`mihomo_first_batch_test.go` 同步。**未新增生产代码文件，未修改前端生产代码。**
- **偏差（4 项）：** ①`node/check.go`、`node/check_test.go`、`node/node_test.go`、`node/sensitive_paths.go`、`node/registry.go` 属 Step 20 文件边界之外，但为满足用户确认的「构造后脱敏」与「evidence canonical path」必须修改；三处原「渲染器只收到 REDACTED」断言按新合同改写为「适配器接收真实参数、响应与预览不泄漏凭据」，安全断言未放宽。②显式 adapter 不再输出 `tls: false`／`skip-cert-verify: false`／`udp: false`／未启用的 `smux: {enabled: false}`／未启用的 `ss-opts`，与固定 tag 的 `omitempty` 语义等价（旧 legacy 透传会写出这些显式默认值）。③首批四协议的 `alpn` 保留逗号分隔字符串兼容（新增 `copyClashALPN`），已迁移协议行为不变。④`CheckClashContent` 的端点／必填问题路径由节点级改为字段级，属诊断精度提升，未新增拒绝条件。
- **定向与联合门禁（全部实际执行）：**
  - `cd backend && go test ./internal/assembly -count=1`：通过（assembly 128 个测试函数，含 Step 20 新增 19 个）。
  - `cd backend && go test ./internal/node -count=1`：通过（node 274 个测试函数）。
  - `cd backend && go test ./... -count=1`：`ok=41`、`no_test_files=5`、`fail=0`。
  - `cd backend && go build ./...`、`go vet ./...`、`gofmt -l ./internal/ ./cmd/`（无输出）、`go run ./cmd/errgate ./...`（`OK (0 ignored errors matched baseline; baseline entries 0; 0 unexpected)`）：通过。
  - `MIHOMO_11931_BIN='/Users/kyle/Desktop/Repo/Temp/mihomo-darwin-arm64-go124-v1.19.31' ./.mihomo-test.sh`：退出码 0；`^TestMihomo11931` 共 17 个测试函数、89 个用例全部通过（正例 37、反例 52），其中首批四协议正例 4 项（`vless-ws-tls`／`vmess-ws-tls`／`trojan-ws-tls`／`ss-aes-gcm`）与反例 4 项（缺 `uuid`／缺 `uuid`／缺 `password`／非法 cipher）在 adapter 迁移后重新通过，证明输出形状变化未影响内核接受度。
  - `node scripts/check-md-links.mjs Build32.md`：`checked_links=2 missing=0 files=1`；`git diff --check`：退出码 0。
- **legacy 计数终值：** `legacyAdapterPendingCount() == 0`，缺失 adapter 集合为空；`legacy_adapter_pending` 已从全部 manual 协议诊断中消失。
- **URI 能力终值：** 11 个协议具备映射、8 个协议对 `sr-subs`／`generic-subs` 均稳定 `target_unsupported`／`skip` 且 `preview` 为空、不返回 500。
- **未执行项：** 本步**未修改前端生产代码**，因此未运行前端全量测试、`vue-tsc` 与前端生产构建（Step 21 未开始）；未运行 `go test -race`、Docker 构建、API／浏览器 smoke、真实客户端导入／连接或用户人工验收；未做 Build32 归档、Design4／AGENTS 同步或 Git 提交。上述证据保留到 Step 21／Step 22。

**独立核验修复（2026-09-23）**

- **已保存凭据检查与正式装配同源：** 独立核验发现 `mergeSensitiveWithOps` 在编辑检查中保留数据库密文，而正式装配会先解密，导致两条路径虽然共用 adapter，却没有使用相同凭据值。`node.Check` 现于只读副本上解密 `enc:v1:` 敏感路径后再执行活动投影、目标校验与 adapter 构造；新草稿明文保持不变，数据库和响应均不写入明文。既有 VLESS 编辑检查测试改为精确断言 adapter 收到原始明文，并继续断言检查前后数据库 JSON、修订、host、port 完全不变。
- **URI 短凭据不再误伤公开字段：** 删除整条 URI 上按原文／转义／Base64 字符串执行的全局 `ReplaceAll`。真实链接仍先用真实凭据构造；预览随后使用同一 `RenderLink` 和敏感字段占位副本重建，因此编码规则仍同源，且不会把一字符密码与 host、节点名或普通参数中的同字符一起替换；对照构造失败时 fail closed，不返回预览。新增 HTTP 一字符密码 `a` 且 host／节点名同时含 `a` 的回归，精确锁定只有凭据组件变为 `REDACTED`。
- **对象列表 canonical path：** `node.SchemaPathExists` 新增严格的非负数字索引解析，`peers[0].allowed-ips`／`peers[12].reserved` 可回指 WireGuard `peers` 列表 schema，非法索引、标量索引和未知子字段被拒绝；目标诊断测试删除复制的本地解析器，直接调用生产函数，避免测试与实现同错。
- **修复后门禁：** `go test ./internal/node ./internal/assembly -count=1` 通过（node 276、assembly 129 个测试函数）；`go test ./... -count=1`、`go build ./...`、`go vet ./...`、`gofmt -l ./internal/ ./cmd/`（无输出）、`go run ./cmd/errgate ./...` 全部通过；固定 Mihomo v1.19.31 门禁仍为 17 个测试函数／89 个用例全部通过。前端生产代码未改，race／Docker／API／浏览器 smoke／真实连接／人工验收仍保留到 Step 21／22。
