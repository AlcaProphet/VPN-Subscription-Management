...
> **文档定位：** 本文是 VPN 订阅管理系统第十六轮当前构建方案，将 [Design3.md](Design3.md) 的已确认设计转化为可逐步执行和验收的实现手册。本文只定义构建计划，尚未开始代码实施。
...
| Step | 内容 | 设计依据 | 状态 |
|------|------|----------|------|
| 1 | Canonical Rule 与中央能力注册表 | Design3 §三、§七 | ☐ 未开始 |
| 2 | 单 URL 单适配器探测、解析与规范化 | Design3 §二、§四、§五 | ☐ 未开始 |
| 3 | 1016 不兼容迁移、来源模型与素材池 CRUD | Design3 §三、§六、§八 | ☐ 未开始 |
| 4 | per-source 快照同步、异常保护与 pending 操作 | Design3 §六 | ☐ 未开始 |
| 5 | 装配目标过滤、转换回执与蓝图失效引用 | Design3 §七、§六.5 | ☐ 未开始 |
| 6 | 素材池 API 与前端三模式交互 | Design3 §八 | ☐ 未开始 |
| 7 | cron、数据清理、概览、备份与跨模块回归 | Design3 §九 | ☐ 未开始 |
| 8 | 全量验证、文档同步与构建收口 | AGENTS.md §3.4～§3.6 | ☐ 未开始 |
