...
- [backend/internal/setup/setup_test.go](file://backend/internal/setup/setup_test.go)
...
## 更新摘要
**所做更改**
- 更新了 Clash Verge 平台配置中的 profile-update-interval 值从 300 小时改为 6 小时
- 添加了相关测试用例的验证逻辑说明
- 补充了平台附加头配置的详细说明
- 更新了部署环境和最佳实践部分
...
**章节来源**
- [backend/internal/config/config.go:24-46](file://backend/internal/config/config.go#L24-46)
- [backend/internal/setup/setup.go:41-141](file://backend/internal/setup/setup.go#L41-141)
- [backend/internal/oidc/oidc.go:23-33](file://backend/internal/oidc/oidc.go#L23-33)
- [backend/internal/mail/mail.go:19-35](file://backend/internal/mail/mail.go#L19-35)
- [backend/internal/server/server.go:62-157](file://backend/internal/server/server.go#L62-L157)
...
**图表来源**
- [backend/internal/config/config.go:94-111](file://backend/internal/config/config.go#L94-111)
- [backend/internal/config/config.go:220-280](file://backend/internal/config/config.go#L220-280)
- [backend/internal/config/config.go:294-313](file://backend/internal/config/config.go#L294-313)
...
**图表来源**
- [backend/internal/setup/setup.go:41-141](file://backend/internal/setup/setup.go#L41-141)
- [backend/internal/setup/setup.go:159-172](file://backend/internal/setup/setup.go#L159-172)
...
### 平台配置与附加头管理
**更新** 修正了 Clash Verge 平台的 profile-update-interval 配置值

- 预设平台：系统预置三个默认平台（Clash Verge、v2rayNG、Shadowrocket），每个平台包含特定的导入协议 scheme 和附加头配置。
- **Clash Verge 平台配置**：
  - 导入协议：`clash://install-config?url={url}`
  - 附加头配置：
    - `Content-Disposition`: `attachment; filename*=UTF-8''subscription.yaml`
    - `profile-update-interval`: `6`（**已更新**：从 300 小时改为 6 小时，符合客户端生态标准）
    - `profile-web-page-url`: `{frontend_url}`
- **附加头作用**：在下载响应中注入这些头部，指导客户端行为（如自动更新间隔、页面跳转等）。
- **测试验证**：setup_test.go 中验证了 Clash Verge 平台的附加头配置正确性。

```mermaid
flowchart TD
Platform["平台配置"] --> ClashVerge["Clash Verge"]
Platform --> V2rayNG["v2rayNG"]
Platform --> Shadowrocket["Shadowrocket"]
ClashVerge --> Headers["附加头配置"]
Headers --> ContentDisp["Content-Disposition"]
Headers --> UpdateInterval["profile-update-interval: 6小时"]
Headers --> WebPage["profile-web-page-url"]
V2rayNG --> EmptyHeaders["无附加头"]
Shadowrocket --> EmptyHeaders
```

**图表来源**
- [backend/internal/setup/setup.go:144-158](file://backend/internal/setup/setup.go#L144-158)
- [backend/internal/setup/setup_test.go:80-87](file://backend/internal/setup/setup_test.go#L80-87)
...
**图表来源**
- [backend/internal/oidc/oidc.go:23-33](file://backend/internal/oidc/oidc.go#L23-33)
- [backend/internal/oidc/oidc.go:75-155](file://backend/internal/oidc/oidc.go#L75-155)
- [backend/internal/oidc/oidc.go:157-212](file://backend/internal/oidc/oidc.go#L157-212)
- [backend/internal/oidc/oidc.go:214-239](file://backend/internal/oidc/oidc.go#L214-239)
- [backend/internal/config/config.go:94-111](file://backend/internal/config/config.go#L94-111)
...
**章节来源**
- [backend/internal/server/server.go:62-157](file://backend/internal/server/server.go#L62-157)
- [backend/internal/server/server.go:160-193](file://backend/internal/server/server.go#L160-193)
...
**章节来源**
- [backend/cmd/server/main.go:40-75](file://backend/cmd/server/main.go#L40-75)
- [backend/internal/server/server.go:160-193](file://backend/internal/server/server.go#L160-193)
...
**章节来源**
- [backend/cmd/server/main.go:24-37](file://backend/cmd/server/main.go#L24-37)
- [backend/cmd/server/main.go:40-75](file://backend/cmd/server/main.go#L40-75)
- [docker-compose.yml.example:19-31](file://docker-compose.yml.example#L19-31)
...
**章节来源**
- [backend/internal/config/config.go:24-37](file://backend/internal/config/config.go#L24-37)
- [backend/internal/config/admin.go:31-49](file://backend/internal/config/admin.go#L31-49)
...
### 平台配置键（platforms 表）
- 基础字段：slug、name、description、schemes、extra_headers。
- **Clash Verge 平台特殊配置**：
  - schemes: `["clash://install-config?url={url}"]`
  - extra_headers: `{"Content-Disposition":"attachment; filename*=UTF-8''subscription.yaml","profile-update-interval":"6","profile-web-page-url":"{frontend_url}"}`
  - **重要更新**：profile-update-interval 已从 300 小时调整为 6 小时，符合客户端生态标准
- v2rayNG 与 Shadowrocket 平台使用空附加头配置
...
- [backend/internal/config/config.go:220-280](file://backend/internal/config/config.go#L220-280)
- [backend/internal/config/config.go:294-313](file://backend/internal/config/config.go#L294-313)
- [backend/internal/config/config.go:335-360](file://backend/internal/config/config.go#L335-360)
