...
- [backend/internal/approval/approval_test.go](file://backend/internal/approval/approval_test.go)
...
## 更新摘要
**变更内容**
- 新增 Xray 实例管理功能：支持多实例管理、节点检测刷新、连通性测试和采集状态监控
- 增强池管理和节点管理：统一节点表设计，支持 manual/xray 双来源节点管理
- 改进代理组管理：新增 group_nodes 关联表，支持节点分配排序和候选集约束
- 高级模式操作增强：提供「开始初始化」按钮进行批量用户同步，支持幂等可重复执行
- 账户对账端点优化：明确预期集合计算应用候选集和可用性过滤，确保与推送集合范围一致
- Go 版本升级至 1.26：支持 xray-core 模块集成，引入 gRPC 管理接口
...
**更新** 本项目已升级至 Go 1.26，支持 Xray-core 集成，新增高级模式操作程序（包括关闭时的尽力而为清理）、公共节点配置和增强的 Xray 实例连接监控能力。**重要变更**：高级模式初始化流程已优化，开启高级模式时不再自动执行推送操作，管理员需通过「开始初始化」按钮手动触发批量用户同步。账户对账端点现已明确预期集合计算应用候选集和可用性过滤，确保与推送集合范围一致。
...
- [Dockerfile:1-31](file://Dockerfile#L1-L31)
- [docker-compose.yml:1-28](file://docker-compose.yml#L1-L28)
- [README.md:65-145](../../../../../README.md#L65-L145)
...
- **Go 版本升级**：构建阶段使用 golang:1.25-alpine，支持 xray-core 模块集成。

```mermaid
flowchart TD
Start(["开始构建"]) --> FE["阶段一: 前端构建<br/>npm ci && npm run build"]
FE --> BE["阶段二: 后端构建<br/>go build -trimpath -ldflags='-s -w'<br/>Go 1.25 + xray-core 支持"]
...
- [backend/internal/config/config.go:24-111](file://backend/internal/config/config.go#L24-L111)
- [README.md:194-218](../../../../../README.md#L194-L218)
...
- [README.md:133-145](../../../../../README.md#L133-L145)
...
**图表来源**
- [backend/internal/log/log.go:21-57](file://backend/internal/log/log.go#L21-57)
...
**章节来源**
- [README.md:165-193](../../../../../README.md#L165-193)
...
**更新** 高级模式初始化流程已优化，现在更加安全和可控。账户对账端点现已明确预期集合计算应用候选集和可用性过滤，确保与推送集合范围一致。
...
- **高级模式操作程序**：
  ...
  - **手动初始化**：**重要变更**：开启高级模式时不再自动执行推送操作，管理员需通过「开始初始化」按钮手动触发批量用户同步
...
- **账户对账优化**：对账端点现在明确应用候选集和可用性过滤，确保与推送集合范围一致，提高账户管理的准确性和一致性。
...
**图表来源**
- [Design2.md:208-214](../../../../../docs/reports/Design/Design2.md#L208-L214)
- [Design2.md:278-286](../../../../../docs/reports/Design/Design2.md#L278-L286)
- [Design2.md:333-350](../../../../../docs/reports/Design/Design2.md#L333-L350)
...
- **账户对账性能**：对账端点应用候选集和可用性过滤，减少不必要的计算和 API 调用。
...
- **高级模式问题**：检查高级模式开关状态、Xray 实例连通性、公共节点配置是否正确；确认是否已通过「开始初始化」按钮触发批量用户同步。
- **账户对账问题**：检查候选集配置、节点可用性设置；确认对账端点是否正确应用过滤逻辑。
...
**更新** 新增的高级模式操作程序、公共节点配置和增强的 Xray 实例监控功能，进一步提升了系统的管理能力、用户体验和运维效率。**特别强调**：高级模式初始化流程已优化，管理员需通过「开始初始化」按钮手动触发批量用户同步，确保配置完成后才执行推送操作。账户对账端点现已明确预期集合计算应用候选集和可用性过滤，确保与推送集合范围一致，提高了账户管理的准确性和一致性。
...
**更新** 高级模式初始化流程已优化，需要手动触发批量用户同步。账户对账端点现已明确预期集合计算应用候选集和可用性过滤。
...
- **手动初始化**：**重要**：开启高级模式后，需通过 Xray 实例页面的「开始初始化」按钮手动触发批量用户同步，确保配置完成后才执行推送操作
- **账户对账配置**：对账端点现已明确应用候选集和可用性过滤，确保与推送集合范围一致
