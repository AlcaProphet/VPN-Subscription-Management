...
## 更新摘要
**变更内容**
- 高级模式初始化流程优化：开启高级模式时不再自动执行推送操作，管理员需手动触发批量初始化
- Xray 实例管理增强：新增「开始初始化」按钮，支持幂等可重复执行的批量用户同步
...
**更新** 本项目已升级至 Go 1.26，支持 Xray-core 集成，新增高级模式操作程序（包括关闭时的尽力而为清理）、公共节点配置和增强的 Xray 实例连接监控能力。**重要变更**：高级模式初始化流程已优化，开启高级模式时不再自动执行推送操作，管理员需通过「开始初始化」按钮手动触发批量用户同步。
...
**更新** 新增的高级模式操作程序、公共节点配置和增强的 Xray 实例监控功能，进一步提升了系统的管理能力、用户体验和运维效率。**特别强调**：高级模式初始化流程已优化，管理员需通过「开始初始化」按钮手动触发批量用户同步，确保配置完成后才执行推送操作。
