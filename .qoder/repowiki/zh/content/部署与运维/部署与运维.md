...
## 更新摘要
**变更内容**
- 账户对账端点文档优化：明确预期集合计算应用候选集和可用性过滤，与推送集合范围保持一致
- Xray 集成增强：新增「开始初始化」按钮支持幂等可重复执行的批量用户同步
- 公共节点配置改进：is_public=1 的节点对所有组自动可见，简化节点管理
- 监控告警完善：Xray 实例连通性检测、采集状态告警、同步状态可视化展示
- Go 版本升级至 1.26，支持 xray-core 模块集成
- 配额管理机制：用户流量配额检查、预验证机制、超限自动移除
