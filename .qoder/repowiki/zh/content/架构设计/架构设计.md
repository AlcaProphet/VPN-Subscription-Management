...
## 更新摘要
**变更内容**
- 新增高级模式切换清理机制，确保关闭高级模式时完整清理Xray相关数据并执行Xray侧账号清理
- 增强订阅模板架构，支持每平台一份订阅模板的模型改进
- 完善用户生命周期同步机制，包括凭据并发首建守卫和超限前置拦截
- 优化Xray集成架构，支持多实例管理和流量配额管控
- 更新数据模型以支持新的装配蓝图和节点管理机制
...
**更新** 本次更新重点增强了高级模式切换清理机制，完善了用户生命周期同步，改进了订阅模板架构，确保系统在高级模式下能够正确管理Xray实例和用户数据的一致性。
...
**更新** 新增了高级模式切换清理机制，确保在关闭高级模式时能够正确清理所有Xray相关数据并执行Xray侧账号清理操作。
...
**更新** 高级模式现在包含完整的切换清理机制，确保在模式切换时数据一致性和完整性。
...
**更新** 高级模式现在包含完整的切换清理机制，确保在关闭高级模式时能够：
- 清理所有Xray实例数据
- 移除source=xray的节点数据
...
### 高级模式切换清理机制
**新增** 高级模式切换清理机制是本次更新的核心功能，确保在关闭高级模式时系统的完整性和一致性：
...
**更新** 现在支持完整的实例管理和对账功能，包括实例级账号对账和一键执行补推。
...
**更新** dataclear服务已扩展支持9张增量新表的清理，确保一键清空功能的完整性。
...
**更新** 前端现在支持高级模式下的Xray实例管理和用户组管理界面。
...
**更新** 配置导入导出功能现在支持Xray实例配置的导入导出，format_version升级为2。
...
**更新** config/export.go现在支持Xray实例配置的导入导出，dataclear/dataclear.go已扩展支持新表的清理。
...
**更新** 高级模式切换清理机制确保在大规模数据清理时的性能和一致性。
...
**更新** 新增高级模式切换相关的故障排查指导，包括清理机制执行状态检查和Xray侧账号清理验证。
...
**更新** 本次更新重点增强了高级模式切换清理机制，完善了用户生命周期同步，改进了订阅模板架构，确保系统在高级模式下能够正确管理Xray实例和用户数据的一致性。Docker多阶段构建与非root运行保障了安全与便携。未来可根据规模需求演进数据层与缓存层，同时保持现有API稳定。系统设计充分考虑了向后兼容性与渐进式升级路径，确保从基础模式到高级模式的平滑过渡。
