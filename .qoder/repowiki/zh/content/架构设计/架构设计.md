...
## 更新摘要
**变更内容**
- 基于Design2.md重大重构完成，移除了大量审计记录和审查文档
- 简化了高级模式行为描述、同步机制和节点管理相关章节
- 提升了文档清晰度和可维护性，聚焦核心架构设计
- 更新了架构图和组件关系以反映简化的设计思路
...
本次更新重点简化了高级模式切换清理机制，优化了用户生命周期同步流程，改进了订阅模板架构，确保系统在高级模式下能够正确管理Xray实例和用户数据的一致性，同时大幅提升了文档的可读性和维护性。
...
本次更新重点简化了高级模式切换清理机制，优化了用户生命周期同步流程，改进了订阅模板架构，确保系统在高级模式下能够正确管理Xray实例和用户数据的一致性，同时大幅提升了文档的可读性和维护性。Docker多阶段构建与非root运行保障了安全与便携。未来可根据规模需求演进数据层与缓存层，同时保持现有API稳定。系统设计充分考虑了向后兼容性与渐进式升级路径，确保从基础模式到高级模式的平滑过渡。
