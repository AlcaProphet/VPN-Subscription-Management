...
## 更新摘要
**变更内容**
- 文档结构重组：将知识库文件从 `.qoder/repowiki/knowledge/zh/` 目录移动到项目根目录，提升架构设计文档的可访问性和组织性
- 保持现有架构设计的完整性和一致性，确保所有技术组件和交互关系得到准确描述
- 更新文档引用路径，反映新的文件组织结构
...
本次更新重点增强了高级模式切换清理机制，完善了用户生命周期同步，改进了订阅模板架构，确保系统在高级模式下能够正确管理Xray实例和用户数据的一致性。
...
新增高级模式切换清理机制，确保在关闭高级模式时能够正确清理所有Xray相关数据并执行Xray侧账号清理操作。
...
高级模式现在包含完整的切换清理机制，确保在模式切换时数据一致性和完整性。
...
高级模式现在包含完整的切换清理机制，确保在关闭高级模式时能够：
- 清理所有Xray实例数据
- 移除source=xray的节点数据
...
### 高级模式切换清理机制
高级模式切换清理机制是本次更新的核心功能，确保在关闭高级模式时系统的完整性和一致性：

**清理流程**：
1. **事务提交前收集**：收集所有xray_users记录（user_id/instance_id/inbound_tag）与各实例连接信息（api_addr/api_tag）
...
现在支持完整的实例管理和对账功能，包括实例级账号对账和一键执行补推。
...
config/export.go现在支持Xray实例配置的导入导出，dataclear/dataclear.go已扩展支持新表的清理。
...
新增高级模式切换相关的故障排查指导，包括清理机制执行状态检查和Xray侧账号清理验证。
...
本次更新重点增强了高级模式切换清理机制，完善了用户生命周期同步，改进了订阅模板架构，确保系统在高级模式下能够正确管理Xray实例和用户数据的一致性。Docker多阶段构建与非root运行保障了安全与便携。未来可根据规模需求演进数据层与缓存层，同时保持现有API稳定。系统设计充分考虑了向后兼容性与渐进式升级路径，确保从基础模式到高级模式的平滑过渡。
