...
## 更新摘要
**变更内容**
- 增强了订阅响应头文档，包括 profile-update-interval 和 profile-web-page-url 的详细说明
- 改进了不同模式下使用响应头的清晰度（基础模式 vs 高级模式）
- 统一了设计规格中的术语一致性
- 更新了响应头注入机制和平台附加头的处理逻辑
...
本次更新重点增强了订阅响应头功能，包括 profile-update-interval 和 profile-web-page-url 的精确控制，改进了不同模式下的响应头行为，确保系统在基础模式和高级模式下的响应头一致性和准确性。
...
简化后的高级模式切换清理机制，确保在关闭高级模式时能够正确清理所有Xray相关数据并执行Xray侧账号清理操作。
...
简化后的高级模式包含更清晰的切换清理机制，确保在模式切换时数据一致性和完整性。
...
**高级模式**（需显式开关解锁）= Xray-core 集成、多用户组管理、流量配额管控、每用户专属订阅

简化后的高级模式包含更清晰的切换清理机制，确保在关闭高级模式时能够：
- 清理所有Xray实例数据
- 移除source=xray的节点数据
...
SwitchCheck --> |关闭| Cleanup["执行清理机制<br/>清理Xray数据"]
Cleanup --> Basic
```
...
**章节来源**
- [Design2.md:260-290](file://Design2.md#L260-290)
...
**章节来源**
- [Design2.md:357-374](file://Design2.md#L357-374)
...
简化后的架构支持完整的实例管理和对账功能，包括实例级账号对账和一键执行补推。
...
**图表来源**
- [Design2.md:218-229](file://Design2.md#L218-L229)
- [Design2.md:305-332](file://Design2.md#L305-332)

**章节来源**
- [Design2.md:189-229](file://Design2.md#L189-229)
- [Design2.md:305-332](file://Design2.md#L305-332)
...
dataclear服务已扩展支持9张增量新表的清理，确保一键清空功能的完整性。
...
配置导入导出功能现在支持Xray实例配置的导入导出，format_version升级为2。
...
- [Dockerfile:1-31](file://Dockerfile#L1-L31)
- [docker-compose.yml:1-28](file://docker-compose.yml#L1-L28)
- [README.md:221-227](file://README.md#L221-227)
- [Design2.md:218-229](file://Design2.md#L218-229)
...
简化后的高级模式切换清理机制确保在大规模数据清理时的性能和一致性。
...
简化后的高级模式切换相关的故障排查指导，包括清理机制执行状态检查和Xray侧账号清理验证。
...
**基础模式**提供完整的订阅管理能力，包括规则素材池、订阅装配拼接、配置生成分发等核心功能。**高级模式**通过Xray-core集成实现多用户VPN服务管理，支持用户生命周期同步、流量配额管控与每用户专属订阅生成。

本次更新重点增强了订阅响应头功能，包括 profile-update-interval 和 profile-web-page-url 的精确控制，改进了不同模式下的响应头行为，确保系统在基础模式和高级模式下的响应头一致性和准确性。同时简化了高级模式切换清理机制，优化了用户生命周期同步流程，改进了订阅模板架构，确保系统在高级模式下能够正确管理Xray实例和用户数据的一致性，同时大幅提升了文档的可读性和维护性。Docker多阶段构建与非root运行保障了安全与便携。未来可根据规模需求演进数据层与缓存层，同时保持现有API稳定。系统设计充分考虑了向后兼容性与渐进式升级路径，确保从基础模式到高级模式的平滑过渡。
