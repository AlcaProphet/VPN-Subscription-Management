...
- [Node-Link-Standards.md](../../../../../docs/Reference/Node-Link-Standards.md)
- [SSpanel-Subscribe.md](../../../../../docs/Reference/SSpanel-Subscribe.md)
- [Clash.yaml.template.md](../../../../../docs/DocTemplates/Clash.yaml.template.md)
- [Shadowrocket.conf.template.md](../../../../../docs/DocTemplates/Shadowrocket.conf.template.md)
- [DailyData.txt.template.md](../../../../../docs/DocTemplates/DailyData.txt.template.md)
- [AGENTS.md](../../../../../AGENTS.md)
...
## 更新摘要
**变更内容**
- Go 运行时版本要求从 1.25 升级到 1.26，以支持 xray-core 依赖
- Docker 构建环境同步升级至 golang:1.26-alpine 基础镜像
- 构建验证流程增强，确保依赖管理的正确性
- 新增 Xray 集成相关的开发指导与配置说明
...
**更新** Go 运行时版本已升级至 1.26，以支持 xray-core 依赖引入，为订阅装配器与 Xray 集成提供技术基础。新增节点链接标准、订阅机制分析和配置模板使用指导，为订阅生成和协议转换提供完整的开发参考。
...
- 后端（Go 1.26）
  ...
  - 依赖声明：backend/go.mod（Go 1.26 要求）
- 前端（Vue 3 + Vite）
...
- 部署与运行
  - 多阶段镜像构建：Dockerfile（golang:1.26-alpine）
...
- [Node-Link-Standards.md:1-144](../../../../../docs/Reference/Node-Link-Standards.md#L1-L144)
- [SSpanel-Subscribe.md:1-200](../../../../../docs/Reference/SSpanel-Subscribe.md#L1-L200)

**章节来源**
- [README.md:230-265](../../../../../README.md#L230-L265)
...
- **新增** 订阅装配器与 Xray 集成
  ...
  - 集成 xray-core 依赖，支持 Xray 实例管理与用户账户同步。
...
- [Node-Link-Standards.md:84-127](../../../../../docs/Reference/Node-Link-Standards.md#L84-L127)

## 架构总览
系统采用"前后端分离 + 单容器打包"的架构：前端由 Vite 构建为静态资源，嵌入后端二进制并通过 Gin 提供静态服务；后端以 Gin 暴露 REST API，SQLite 作为嵌入式数据库，所有数据持久化在 /data 卷中。

**更新** 新增订阅装配器架构，支持 Clash YAML 和 Shadowrocket 配置的自动生成，基于标准化的节点链接协议。Go 运行时升级至 1.26 以支持 xray-core 依赖，为 Xray 集成提供基础能力。
...
**图表来源**
- [Node-Link-Standards.md:21-43](../../../../../docs/Reference/Node-Link-Standards.md#L21-L43)
- [Node-Link-Standards.md:84-127](../../../../../docs/Reference/Node-Link-Standards.md#L84-L127)
...
- **新增** Go 版本相关问题
  - 现象：编译错误或依赖解析失败
  - 处理：确认使用 Go 1.26+ 版本；检查 go.mod 中的 Go 版本要求；清理并重新下载依赖

**章节来源**
- [backend/cmd/server/main.go:24-99](file://backend/cmd/server/main.go#L24-L99)
- [backend/internal/server/server.go:160-193](file://backend/internal/server/server.go#L160-L193)
- [backend/internal/auth/auth.go:118-193](file://backend/internal/auth/auth.go#L118-L193)
- [frontend/vite.config.ts:8-15](file://frontend/vite.config.ts#L8-L15)
- [Node-Link-Standards.md:63-83](../../../../../docs/Reference/Node-Link-Standards.md#L63-L83)
- [backend/go.mod:3](file://backend/go.mod#L3)

## 结论
本项目以"单容器、零外部依赖、易部署"为目标，后端采用 Gin + SQLite 的轻量架构，前端使用 Vue 3 + Vite 提供现代体验。通过版本化迁移、配置加密、会话鉴权与完善的日志/监控能力，兼顾了安全性与可维护性。新增的订阅装配器和协议兼容性分析为代理订阅管理提供了完整的技术支撑。

**更新** Go 运行时升级至 1.26 以支持 xray-core 依赖，为 Xray 集成和高级订阅功能奠定基础。新增的参考文档和模板文件为订阅生成和协议转换提供了详细的实现指导，确保了与主流代理生态的互操作性。遵循本指南的流程与规范，可高效开展功能开发与协作贡献。

## 附录：常用命令与环境配置

- 本地开发
  - **环境要求**：Go 1.26+，Node.js 18+
...
- IDE 建议
  - Go：安装 Go 1.26+ 工具链与 gopls，开启 go mod tidy 与 go vet 检查。
...
- [frontend/package.json:6-11](file://frontend/package.json#L6-L11)
- [Node-Link-Standards.md:84-127](../../../../../docs/Reference/Node-Link-Standards.md#L84-L127)
- [Clash.yaml.template.md:1-321](../../../../../docs/DocTemplates/Clash.yaml.template.md#L1-L321)
- [Shadowrocket.conf.template.md:1-47](../../../../../docs/DocTemplates/Shadowrocket.conf.template.md#L1-L47)
- [.smoke-test.sh:1-73](file://.smoke-test.sh#L1-L73)
- [backend/go.mod:3](file://backend/go.mod#L3)
