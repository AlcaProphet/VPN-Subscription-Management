...
## 更新摘要
**变更内容**
- 新增高级功能表结构：rule_pools、pool_entries、xray_instances、proxy_groups、group_nodes、assembly_blueprints
- 现有表加密增强：users表新增AES-256-GCM加密字段（uuid_encrypted、proxy_secret_encrypted），nodes表protocol_json凭据加密存储
- 移除遗留表：删除group_selections和subscription_group_rel，改用新的组节点分配机制
- 平台表多安装包升级：从单值列迁移到JSON数组结构（installer_files/installer_urls）
- 数据模型重构：从订阅选择模式转向节点分配模式，支持Xray实例管理和流量统计
...
**最新更新**：系统已升级为高级模式，新增Xray实例管理、规则素材池、代理组配置等功能，同时增强了敏感数据的AES-256-GCM加密存储，移除了传统的订阅选择机制，采用更灵活的节点分配模式。
...
### 订阅与版本
- 订阅表 subscriptions：slug 全局唯一、名称不强制唯一、平台外键、当前版本号、时间戳。**改造**：「平台 + 产物类型（yaml/subs）」唯一约束。
...
- **xray_users表**：记录用户与Xray实例的关联关系及同步状态
...
- **group_nodes表**：替代原有的group_selections，支持组与节点的灵活分配和排序
...
- 索引设计：
  ...
  - **新增**：xray_users(user_id, instance_id)
  - **新增**：traffic_records(user_id, ym)

```mermaid
graph LR
Users["users<br/>加密增强"] --> Groups["groups<br/>默认配额"]
Platforms["platforms<br/>多安装包JSON"] --> Subscriptions["subscriptions<br/>产物类型约束"]
...
XrayInstances --> XrayUsers["xray_users"]
Users --> XrayUsers
Groups --> GroupNodes["group_nodes"]
Nodes --> GroupNodes
...
**最新改进**：系统已成功升级为高级模式，新增了Xray实例管理、规则素材池、代理组配置等高级功能，同时增强了敏感数据的AES-256-GCM加密存储。平台表已完成从单值安装包到多安装包JSON数组结构的迁移，支持同一平台配置多个本地安装包和外部下载链接。向后兼容的迁移策略确保了现有数据的无缝转换，提升了系统的分发能力和用户体验。
...
**更新**：第1008号迁移实现了从单值到JSON数组的平滑过渡，确保数据完整性和向后兼容性。高级功能相关的迁移将遵循相同的版本管理策略。
