...
## 更新摘要
**变更内容**
- 新增规则素材池系统：支持URL同步与手工维护，智能去重机制
- 增强规则解析能力：支持多种输入格式的健壮解析
- 完善验证框架：条目白名单校验、来源隔离、差量更新
- 性能优化：支持数万条规则条目的高效处理
- 双模式创建增强：文本模式与文件上传模式的智能切换
...
- **规则素材池服务 pool.Service**：负责素材池的URL同步、手工维护、条目解析与去重
- **装配服务 assembly.Service**：负责Clash YAML与Shadowrocket配置的装配拼接
...
participant PS as "素材池服务 pool.Service"
...
API-->>FE : "200 OK"
Note over PS,DB : "素材池同步：URL拉取→解析→UPSERT→差量删除"
```
...
### 规则素材池服务（pool.Service）
- 职责
  - **素材池管理**：创建、更新、删除素材池，管理挂接的URL列表
  - **URL同步**：定时或手动触发，拉取各URL内容，逐行解析为规则条目
  - **手工维护**：管理员手动增删改条目，支持全部规则类型
  - **智能去重**：按（规则类型 + 匹配值）去重合并，避免重复条目
  - **来源隔离**：url来源与manual来源分离，URL同步不影响manual条目
  - **差量更新**：仅对url来源条目做差量更新，源中消失的行自动删除

- 解析规则
  - `full:<域名>` → DOMAIN 条目
  - 裸域名 → DOMAIN-SUFFIX 条目  
  - 标准规则行（如 `IP-CIDR,1.2.3.0/24`）→ 分行直接导入
  - 无法识别的行 → 跳过并记录原因
...
RuleHandler --> PoolService : "调用"
RuleHandler --> AssemblyService : "调用"
```
...
- **素材池性能优化**
  ...
  - 批量UPSERT操作，事务化写入
  - 分页展示，避免整表加载
  - 去重索引 UNIQUE(pool_id, rule_type, match_value)
...
## 结论
本规则引擎实现了分流规则的完整生命周期管理，包括创建、版本化、预览、Token 轮替与删除。**新增的规则素材池系统**提供了强大的URL同步与手工维护能力，支持智能去重、来源隔离和差量更新，能够高效处理数万条规则条目。通过版本服务与 Token 服务的协作，确保了内容的一致性与安全性。前端提供了直观的管理与用户界面，便于日常运维与分发。
