...
## 更新摘要
**变更内容**
- 新增规则素材池系统（rule_pools、pool_entries 表）
- Shadowrocket 双输出格式支持（节点订阅 subs + 分流规则 conf）
- 增强配置组装功能，支持三种输出格式（Clash YAML、Shadowrocket subs/conf）
- Xray-core 集成高级模式支持
- 新增装配蓝图（assembly_blueprints）和代理组管理（proxy_groups、group_nodes）
...
**更新** 系统现已支持规则素材池管理、Shadowrocket 双输出格式、增强配置组装和 Xray-core 高级模式集成，提供更强大的订阅管理和配置生成能力。
...
- 规则引擎：分流规则 CRUD、全局共享 Token、版本管理、刷新、**规则素材池管理**。
- 分享系统：分享订阅 CRUD、Token 轮替与吊销、版本管理。
...
- [backend/internal/version/version.go:127-180](file://backend/internal/version/version.go#L127-180)
...
图表来源
- [backend/internal/group/group.go:84-141](file://backend/internal/group/group.go#L84-141)
- [backend/internal/group/group.go:179-220](file://backend/internal/group/group.go#L179-220)
...
### 规则引擎（增强版 - 含素材池）
- 职责：分流规则 CRUD、全局共享 Token、版本管理、刷新、**规则素材池管理**。
- 实现要点：
  - 名称必填，客户端类型当前仅支持 shadowrocket。
  - 标识 slug 全局唯一（跨四类资源），自动生成 rule- 前缀。
...
图表来源
- [backend/internal/rule/rule.go:52-102](file://backend/internal/rule/rule.go#L52-102)
- [backend/internal/version/version.go:127-180](file://backend/internal/version/version.go#L127-180)
...
**更新** 新增的规则素材池系统、Shadowrocket 双输出格式、增强配置组装和 Xray-core 高级模式集成，大幅提升了系统的功能完整性和用户体验。整体设计注重事务一致性、文件操作安全与系统可靠性，适合团队自托管部署与日常运维。
...
- 刷新规则 Token
  ...
  - 响应：{ "token": "..." }
  - 参考：[backend/internal/rule/rule.go:131-134](file://backend/internal/rule/rule.go#L131-L134), [backend/internal/token/token.go:210-224](file://backend/internal/token/token.go#L210-224)
