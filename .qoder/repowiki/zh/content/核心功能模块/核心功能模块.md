...
B --> I["group.go<br/>用户组/每平台选定"]
B --> J["rule.go<br/>分流规则/Token"]
...
- 用户组管理：组 CRUD、每平台选定订阅、变更影响提示、删组迁入默认组。
- 平台管理：平台资源 CRUD、scheme/附加头校验、安装包流式上传与覆盖、级联删除。
- 规则引擎：分流规则 CRUD、全局共享 Token、版本管理与刷新。
...
### 用户组管理
- 职责：用户组 CRUD、每平台选定订阅、关联约束、删组迁入默认组。
- 实现要点：
  - 名称全局唯一，slug 自动生成。
  - 更新时校验取消关联的订阅是否仍被本次选定引用，防止悬空选定。
  - 重建 group_selections，确保选定必须在关联范围内。
...
```mermaid
flowchart TD
Start(["开始"]) --> UpdateGroup["更新组(名称/关联/选定)"]
UpdateGroup --> DiffRemoved["计算将被移除的关联订阅"]
DiffRemoved --> CheckSelection{"新选定是否引用被移除项?"}
CheckSelection --> |是| ErrInSel["拒绝：该订阅正被设为组内某平台的默认订阅"]
CheckSelection --> |否| RebuildRel["重建订阅-组关联"]
RebuildRel --> RebuildSel["重建 group_selections(校验范围)"]
RebuildSel --> ClearFlag["清除 needs_reselect 标记"]
ClearFlag --> End(["完成"])
```
...
### 规则引擎（分流规则）
- 职责：分流规则 CRUD、全局共享 Token、版本管理、Token 刷新。
- 实现要点：
...
## 结论
本系统通过清晰的分层架构与模块化设计，实现了订阅管理、认证授权、用户组、平台、规则、分享等核心功能。版本管理组件为多类资源提供一致的能力，Token 管理确保下载安全与生命周期可控。配置中心提供灵活的运行时配置与敏感数据保护。整体设计注重事务一致性、文件操作安全与系统可靠性，适合团队自托管部署与日常运维。
...
- 平台安装包上传
  ...
  - 表单字段：file
  - 响应：{ "message": "success" }
  - 参考：[backend/internal/platform/platform.go:269-334](file://backend/internal/platform/platform.go#L269-L334)
