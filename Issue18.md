...
> **文档定位：** 本文档记录 2026-09-16 发现的邮件与站点信息问题。R32-01 记录站点名称一致性修复与自动化验收；R32-02 记录业务邮件同步发送阻塞主流程的问题、已确认并进一步完善的实施方案与验收合同。已实施的邮件内容设计仍见 [Design5.md](Design5.md) §7，历史构建记录仍以 [Build28.md](docs/reports/Build/Build28.md) 为准。
...
1. **公共忘记密码内部故障状态码：** 推荐保留当前 500，仅不存在邮箱、无本地密码、邮件不可用和入队拒绝保持统一 200。优点是令牌随机数/数据库故障仍可由调用方和监控发现；选择全量 200 会减少可观察差异，但会把真实内部故障伪装成正常受理。
2. **管理员单用户入队拒绝后的 token：** 推荐按 token 精确 best-effort 删除本次新建记录，再返回 503；删除失败仅记录安全告警并等待现有过期清理。另一方案是保留 token 至过期，实现较简单，但会留下用户无法取得的有效 token。

- **当前状态：** 🟡 R32-02 主体方案已进一步完善，等待 2.12 两项用户决策后转为实施定稿；尚未修改实现代码，未执行实现后门禁，也未授权开始代码 Step（2026-09-17）。
